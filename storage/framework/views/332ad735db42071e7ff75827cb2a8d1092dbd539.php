...
/* DataTable wrapper overflow fixes */
.dataTables_wrapper {
  overflow: visible !important;
}
...
/* Bootstrap dropdown fixes for DataTables */
.table-responsive {
  overflow: visible !important;
}
...
<section class="content">
    ...
    <?php if($type == 'customer'): ?>
        <div class="col-md-3">
            <div class="form-group">
                ...
                <?php echo Form::select('has_no_sell_from', ['one_month' => __('lang_v1.one_month'), 'three_months' => __('lang_v1.three_months'), 'six_months' => __('lang_v1.six_months'), 'one_year' => __('lang_v1.one_year')], null, ['class' => 'form-control', 'id' => 'has_no_sell_from', 'placeholder' => __('messages.please_select')]); ?>
    ...
    <div class="col-md-3">
        <div class="form-group">
            ...
            <?php echo Form::select('status_filter', ['active' => __('business.is_active'), 'inactive' => __('lang_v1.inactive')], null, ['class' => 'form-control', 'id' => 'status_filter', 'placeholder' => __('lang_v1.none')]); ?>
...
$(document).ready(function() {
    ...
    $(document).on('shown.bs.dropdown', '.dropdown', function() {
        ...
        if ($table.length) {
            // Ensure dropdown appears above scrollable content
            $dropdown.css({
                'position': 'fixed',
                'z-index': '9999'
            });
    ...
    $(document).on('click', '.generate-payment-token', function(e) {
        e.preventDefault();
        ...
        $.post('/contacts/' + contactId + '/generate-payment-token', {
        ...
        .done(function(response) {
            if (response.success) {
                    ...
                    '<div class="modal-dialog" role="document">' +
                        '<div class="modal-content">' +
                            '<div class="modal-header">' +
                                '<h4 class="modal-title">Payment Link Generated</h4>' +
                                '<button type="button" class="close" data-dismiss="modal">' +
                                    '<span>&times;</span>' +
                                '</button>' +
                            '</div>' +
                            '<div class="modal-body">' +
                                '<p><strong>Payment link has been generated successfully!</strong></p>' +
                                '<div class="form-group">' +
                                    '<label>Payment Link:</label>' +
                                    '<div class="input-group">' +
                                        '<input type="text" class="form-control" id="generated-link" value="' + response.payment_link + '" readonly>' +
                                        '<span class="input-group-btn">' +
                                            '<button class="btn btn-default copy-link-btn" type="button">' +
                ...
                // Handle copy button
                modal.on('click', '.copy-link-btn', function() {
                ...
                modal.on('hidden.bs.modal', function() {
                    modal.remove();
                    // Reload table to show new action buttons
        ...
        .fail(function() {
            toastr.error('Failed to generate payment token');
        })
        .always(function() {
            $this.html('<i class="fas fa-plus-circle"></i> Generate Payment Link');
        });
    });
    
    // Copy Payment Link
    $(document).on('click', '.copy-payment-link', function(e) {
        e.preventDefault();
        var contactId = $(this).data('contact-id');
        
        $.get('/contacts/' + contactId + '/payment-link')
        .done(function(response) {
            if (response.success) {
                navigator.clipboard.writeText(response.payment_link).then(function() {
                    toastr.success('Payment link copied to clipboard!');
                }).catch(function() {
                    // Fallback for older browsers
                    var textArea = document.createElement("textarea");
                    textArea.value = response.payment_link;
        ...
        .fail(function() {
            toastr.error('Failed to get payment link');
        });
    });
    
    // View Payment Link Details
    $(document).on('click', '.view-payment-link', function(e) {
        e.preventDefault();
        var contactId = $(this).data('contact-id');
            ...
            if (response.success) {
                var modal = $('<div class="modal fade" tabindex="-1" role="dialog">' +
                    '<div class="modal-dialog" role="document">' +
                        '<div class="modal-content">' +
                            '<div class="modal-header">' +
                                '<h4 class="modal-title">Payment Link Details</h4>' +
                                '<button type="button" class="close" data-dismiss="modal">' +
                                    '<span>&times;</span>' +
                                '</button>' +
                            '</div>' +
                            '<div class="modal-body">' +
                                '<div class="form-group">' +
                                    '<label>Payment Link:</label>' +
                                    '<div class="input-group">' +
                                        '<input type="text" class="form-control" value="' + response.payment_link + '" readonly>' +
                                        '<span class="input-group-btn">' +
                                            '<button class="btn btn-default copy-detail-link-btn" type="button">' +
                                                '<i class="fas fa-copy"></i> Copy' +
                                            '</button>' +
                                        '</span>' +
                                    '</div>' +
                                ...
                                '<div class="row">' +
                                    '<div class="col-md-6">' +
                                        '<strong>Has Outstanding Invoices:</strong> ' + (response.has_outstanding ? 'Yes' : 'No') +
                                    '</div>' +
                                    '<div class="col-md-6">' +
                                        '<strong>Total Outstanding:</strong> ' + (response.total_outstanding || '0.00') +
                                    '</div>' +
                                '</div>' +
                            '</div>' +
                            '<div class="modal-footer">' +
                                '<a href="' + response.payment_link + '" target="_blank" class="btn btn-primary">' +
                ...
                // Handle copy button
                modal.on('click', '.copy-detail-link-btn', function() {
                    var link = response.payment_link;
        ...
        .fail(function() {
            toastr.error('Failed to get payment link details');
        });
    });
});
</script>
