            ...
            <div class="nav-tabs-custom">
                ...
                <div class="tab-content">
                    <div class="tab-pane active" id="product_list_tab">
                        <?php if($is_admin): ?>
                            <a class="btn btn-success pull-right margin-left-10" href="<?php echo e(action([\App\Http\Controllers\ProductController::class, 'downloadExcel']), false); ?>"><i class="fa fa-download"></i> <?php echo app('translator')->get('lang_v1.download_excel'); ?></a>
                        <?php endif; ?>
                        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('product.create')): ?>                            
                            <a class="btn btn-primary pull-right" href="<?php echo e(action([\App\Http\Controllers\ProductController::class, 'create']), false); ?>">
        ...
        $('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
            if ($(e.target).attr('href') == '#product_stock_report') {
                if (typeof stock_report_table !== 'undefined') {
                    // Override the data function to use products page filters
                    var table_settings = stock_report_table.settings()[0];
                    var original_ajax_data = table_settings.ajax.data;
                    table_settings.ajax.data = function(d) {
                        // Call original data function first if it exists
                        if (typeof original_ajax_data === 'function') {
                            original_ajax_data(d);
                        }
                        // Override with products page filter values
                        d.location_id = $('#location_id').val();
                        d.category_id = $('#product_list_filter_category_id').val();
                        d.brand_id = $('#product_list_filter_brand_id').val();
                        d.unit_id = $('#product_list_filter_unit_id').val();
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/resources/views/product/index.blade.php ENDPATH**/ ?>