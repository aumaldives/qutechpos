...
<tr class="product_row" data-row_index="<?php echo e($row_count, false); ?>" <?php if(!empty($so_line)): ?> data-so_id="<?php echo e($so_line->transaction_id, false); ?>" <?php endif; ?>>
	<td>
		...
		<?php
			...
			if(session()->get('business.enable_avl_quantity_pos') == 1){
				$product_qty_available = '<br/><strong> Available Quantity: ' . $product->qty_available . '</strong>';
			}
			$product_name = $product->product_name . '<br/>' . $product->sub_sku . $product_qty_available;
		...
		<?php
			...
			$hide_tax_1 = 'hide';
	        if(session()->get('business.enable_exc_tax') == 1){
	            $hide_tax_1 = '';
	        }
	...
	<td>
		...
		<?php if(count($sub_units) > 0): ?>
			<br>
			<select name="products[<?php echo e($row_count, false); ?>][sub_unit_id]" class="form-control input-sm sub_unit new_sub_unit">
                <?php $__currentLoopData = $sub_units; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $value): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($key, false); ?>" data-multiplier="<?php echo e($value['multiplier'], false); ?>" data-unit_name="<?php echo e($value['name'], false); ?>" data-allow_decimal="<?php echo e($value['allow_decimal'], false); ?>" data-custom_unit="<?php echo e($value['custom_unit'], false); ?>" data-custom_unit_price="<?php echo e($value['custom_unit_price'], false); ?>" <?php if(!empty($product->sub_unit_id) && $product->sub_unit_id == $key): ?> selected <?php endif; ?>>
	...
	<?php if(!empty($is_direct_sell)): ?>
		...
		<td <?php if(!$edit_discount): ?> class="hide" <?php endif; ?>>
			<?php echo Form::text("products[$row_count][line_discount_amount]", number_format($discount_amount, session('business.currency_precision', 2), session('currency')['decimal_separator'], session('currency')['thousand_separator']), ['class' => 'form-control input_number row_discount_amount']); ?><br>
	...
	<?php
		...
		?>
		<!-- price exc tax-->
<!-- Main page unit price in a td element -->
<td class="<?php if(!auth()->user()->can('edit_product_price_from_sale_screen')): ?> hide <?php endif; ?> main_page_unit_price_td <?php echo e($hide_tax_1, false); ?>">
    <input type="text" name="products[<?php echo e($row_count, false); ?>][unit_price]" class="form-control pos_unit_price input_number mousetrap main_page_unit_price" value="<?php echo e(@number_format($pos_unit_price, 2, '.', ''), false); ?>" <?php if(!empty($pos_settings['enable_msp'])): ?> data-rule-min-value="<?php echo e($pos_unit_price, false); ?>" data-msg-min-value="<?php echo e(__('lang_v1.minimum_selling_price_error_msg', ['price' => number_format($pos_unit_price, session('business.currency_precision', 2), session('currency')['decimal_separator'], session('currency')['thousand_separator'])]), false); ?>" <?php endif; ?>>
 <?php if(!empty($last_sell_line)): ?>
        <br>
        <small class="text-muted"><?php echo app('translator')->get('lang_v1.prev_unit_price'); ?>: <?php 
            $formated_number = "";
...
</tr><?php /**PATH /var/www/html/resources/views/sale_pos/product_row.blade.php ENDPATH**/ ?>