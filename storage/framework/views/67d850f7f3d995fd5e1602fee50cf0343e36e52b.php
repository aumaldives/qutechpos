...
<div class="row">
	<div class="pos-form-actions">
		<div class="col-md-12">
			...
			<?php endif; ?>
			<button type="button" class="<?php if($is_mobile): ?> col-xs-6 <?php endif; ?> btn bg-info text-white btn-default btn-flat <?php if($pos_settings['disable_draft'] != 0): ?> hide <?php endif; ?>" id="pos-draft" <?php if(!empty($only_payment)): ?> disabled <?php endif; ?>><i class="fas fa-file-alt"></i> <?php echo app('translator')->get('sale.draft'); ?></button>
			<button type="button" class="btn btn-default bg-yellow btn-flat <?php if($is_mobile): ?> col-xs-6 <?php endif; ?>" id="pos-quotation" <?php if(!empty($only_payment)): ?> disabled <?php endif; ?>><i class="fas fa-file-invoice"></i> <?php echo app('translator')->get('lang_v1.quotation'); ?></button>
			...
			<?php if(empty($pos_settings['disable_credit_sale_button'])): ?>
				...
				title="<?php echo app('translator')->get('lang_v1.tooltip_credit_sale'); ?>" <?php if(!empty($only_payment)): ?> disabled <?php endif; ?>>
					<i class="fas fa-handshake" aria-hidden="true"></i> <?php echo app('translator')->get('lang_v1.credit_sale'); ?>
				</button>
			<?php endif; ?>
			<button type="button" 
				class="btn bg-maroon btn-default btn-flat no-print <?php if(!empty($pos_settings['disable_suspend'])): ?> <?php endif; ?> pos-express-finalize <?php if(!array_key_exists('card', $payment_types)): ?> hide <?php endif; ?> <?php if($is_mobile): ?> col-xs-6 <?php endif; ?>" 
			...
			<button type="button" class="btn btn-success <?php if(!$is_mobile): ?> <?php endif; ?> btn-flat no-print <?php if($pos_settings['disable_express_checkout'] != 0 || !array_key_exists('bank_transfer', $payment_types)): ?> hide <?php endif; ?> pos-express-finalize <?php if($is_mobile): ?> col-xs-6 <?php endif; ?>" data-pay_method="bank_transfer" title="<?php echo app('translator')->get('tooltip.express_checkout'); ?>" style="background-color: #74348c; border:1px solid #74348c"><i class="fas fa-university"></i> Bank Transfer</button>
			<!-- Nabeel -->

			<?php if(empty($edit)): ?>
				<button type="button" class="btn btn-danger btn-flat <?php if($is_mobile): ?> col-xs-6 <?php else: ?> btn-xs <?php endif; ?>" id="pos-cancel"> <i class="fas fa-times-circle"></i> <?php echo app('translator')->get('sale.cancel'); ?></button>
			<?php else: ?>
				<button type="button" class="btn btn-danger btn-flat hide <?php if($is_mobile): ?> col-xs-6 <?php else: ?> btn-xs <?php endif; ?>" id="pos-delete" <?php if(!empty($only_payment)): ?> disabled <?php endif; ?>> <i class="fas fa-trash-alt"></i> <?php echo app('translator')->get('messages.delete'); ?></button>
			<?php endif; ?>

			<?php if(!$is_mobile): ?>
			<div class="bg-navy pos-total text-white" style="margin-top:5px;">
			...
			<button type="button" class="pull-right btn btn-primary btn-flat <?php if($is_mobile): ?> col-xs-6 <?php endif; ?>" data-toggle="modal" data-target="#recent_transactions_modal" id="recent-transactions" style="margin-top:5px;"> <i class="fas fa-history"></i> <?php echo app('translator')->get('lang_v1.recent_transactions'); ?></button>
			<?php endif; ?>
...
<?php echo $__env->make('sale_pos.partials.edit_plasticbag_modal', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/resources/views/sale_pos/partials/pos_form_actions.blade.php ENDPATH**/ ?>