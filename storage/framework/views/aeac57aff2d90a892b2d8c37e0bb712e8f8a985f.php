	...
	<div class="col-md-12">
		<table class="table table-condensed">
			...
			<tr>
				...
				<td>
					<span>
						<b><?php echo app('translator')->get('Plastic Bag Fees'); ?>(+): <?php
                if(session('business.enable_tooltip')){
                    echo '<i class="fa fa-info-circle text-info hover-q no-print " aria-hidden="true" 
                    data-container="body" data-toggle="popover" data-placement="auto bottom" 
                    data-content="' . __('tooltip.plastic_bags') . '" data-html="true" data-trigger="hover"></i>';
                }
                ?></b>
						<i class="fas fa-edit cursor-pointer"  title="<?php echo app('translator')->get('Plastic Bag Selection'); ?>" aria-hidden="true" data-toggle="modal" data-target="#posPlasticbagModal"></i>
						<span id="plasticbag_charges_amount">0</span>
						<input type="hidden" name="plasticbag_charges" id="plasticbag_charges" value="<?php if(empty($edit)): ?><?php echo e(number_format(0.00, session('business.currency_precision', 2), session('currency')['decimal_separator'], session('currency')['thousand_separator']), false); ?> <?php else: ?><?php echo e(number_format($transaction->plasticbag_charges ?? 0.00, session('business.currency_precision', 2), session('currency')['decimal_separator'], session('currency')['thousand_separator']), false); ?> <?php endif; ?>" data-default="0.00">
					</span>
				</td>
...
</div><?php /**PATH /var/www/html/resources/views/sale_pos/partials/pos_form_totals.blade.php ENDPATH**/ ?>