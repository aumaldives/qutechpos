...
// Smart table dropdown positioning
$(document).ready(function() {
  $(document).on('click', 'table .dropdown-toggle', function() {
    var $btnGroup = $(this).closest('.btn-group');
    var $row = $btnGroup.closest('tr');
    var $table = $btnGroup.closest('table');
    
    if ($row.length && $table.length) {
      var $allRows = $table.find('tbody tr').length ? $table.find('tbody tr') : $table.find('tr');
      var rowIndex = $allRows.index($row);
      var totalRows = $allRows.length;
      
      // For tables with >15 rows, bottom half opens upward
      if (totalRows > 15 && rowIndex >= Math.floor(totalRows / 2)) {
  ...
  // Fix payment modal width on show
  $('#modal_payment').on('show.bs.modal', function() {
    setTimeout(function() {
      $('#modal_payment .modal-dialog').css({
        'width': '95%',
        'max-width': '1200px',
        'margin': '30px auto'
      });
