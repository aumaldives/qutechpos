...
$(document).on('click', '.add_imei_numbers', function() {
    var row = $(this).data('row');
    var product_name = $(this).data('product');
    var quantity_field = $('input[name="purchases[' + row + '][quantity]"]');
    var quantity = parseFloat(quantity_field.val()) || 0;
    
    if (quantity <= 0) {
        toastr.error('Please enter quantity first');
        return;
    }
    
    // Set modal data
    $('#imei_modal').data('row', row);
    $('#imei_quantity').val(Math.floor(quantity));
    $('#product_name_display').text(product_name);
    
    // Load existing IMEIs if any
    var existing_imeis = $('input[name="purchases[' + row + '][imeis]"]').val();
    if (existing_imeis) {
        var imei_array = existing_imeis.split(',').filter(function(imei) {
            return imei.trim() !== '';
        });
        
        if (imei_array.length > 0) {
            generateImeiFields(Math.floor(quantity), imei_array);
        } else {
            $('#imei_fields_container').empty();
        }
    } else {
        $('#imei_fields_container').empty();
    }
    
    $('#imei_modal').modal('show');
});

$(document).on('click', '#generate_imei_fields', function() {
    var quantity = parseInt($('#imei_quantity').val()) || 0;
    generateImeiFields(quantity);
});

function generateImeiFields(quantity, existing_values = []) {
    var container = $('#imei_fields_container');
    container.empty();
    ...
    for (var i = 0; i < quantity; i++) {
        var value = existing_values[i] || '';
        var fieldHtml = '<div class="form-group">' +
            '<label>IMEI ' + (i + 1) + ':</label>' +
            '<input type="text" class="form-control imei-field" data-index="' + i + '" value="' + value + '" placeholder="Enter IMEI number">' +
            '</div>';
        container.append(fieldHtml);
    }
}

$(document).on('click', '#save_imei_numbers', function() {
    var row = $('#imei_modal').data('row');
    var imei_values = [];
    var has_error = false;
    
    // Collect IMEI values
    $('#imei_fields_container .imei-field').each(function() {
        var imei = $(this).val().trim();
        if (imei !== '') {
            // Check for duplicates within this entry
            if (imei_values.includes(imei)) {
                toastr.error('Duplicate IMEI found: ' + imei);
                has_error = true;
                return false;
            }
            
            imei_values.push(imei);
        }
    });
    ...
    // Save IMEIs to hidden field
    var imei_string = imei_values.join(',');
    $('input[name="purchases[' + row + '][imeis]"]').val(imei_string);
    
    // Update display in IMEI container
    var imei_container = $('#imei_container_' + row);
    if (imei_values.length > 0) {
        var display_html = '<small class="text-info">' + 
            '<i class="fa fa-barcode"></i> ' + imei_values.length + ' IMEI(s) added' +
            '</small>';
        imei_container.html(display_html);
    } else {
        imei_container.empty();
    }
    
    $('#imei_modal').modal('hide');
    
    if (imei_values.length > 0) {
        toastr.success(imei_values.length + ' IMEI numbers saved');
    }
});

// Update IMEI modal when quantity changes
$(document).on('change', '.purchase_quantity', function() {
    var row = $(this).closest('tr').index();
    var new_quantity = Math.floor(parseFloat($(this).val()) || 0);
    var imei_input = $('input[name="purchases[' + row + '][imeis]"]');
    var existing_imeis = imei_input.val();
    ...
    if (existing_imeis) {
        ...
        // If quantity decreased, warn about IMEI loss
        if (new_quantity < imei_array.length) {
            if (new_quantity === 0) {
                // Clear all IMEIs
                imei_input.val('');
                $('#imei_container_' + row).empty();
            } else {
                // Truncate IMEIs to match new quantity
                var truncated_imeis = imei_array.slice(0, new_quantity);
                imei_input.val(truncated_imeis.join(','));
                
                var display_html = '<small class="text-info">' + 
                    '<i class="fa fa-barcode"></i> ' + truncated_imeis.length + ' IMEI(s) added' +
                    '</small>';
                $('#imei_container_' + row).html(display_html);
                
                toastr.warning('IMEI count adjusted to match quantity');
            }
        }
    }
});

// Clear IMEI data when row is removed
$(document).on('click', '.remove_purchase_entry_row', function() {
    var row = $(this).closest('tr');
    var row_index = row.index();
    
    // Clear any IMEI data
    $('input[name="purchases[' + row_index + '][imeis]"]').val('');
    $('#imei_container_' + row_index).empty();
});
