// Safe jQuery initialization function
...
// Safe Select2 initialization for price group
function initPriceGroupSelect2() {
    safeJQuery(function() {
        // Only apply Select2 on desktop devices
        var isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
        var isTouch = ('ontouchstart' in window) || (navigator.maxTouchPoints > 0) || (navigator.msMaxTouchPoints > 0);
        var isMobileSize = window.innerWidth <= 1024 || (window.innerWidth <= 1200 && window.innerHeight <= 800);
        
        var isTouchDevice = isTouch && isMobileSize;
        
        if (!isTouchDevice && $('#price_group').length) {
            // Desktop: Use Select2
            $('#price_group').removeClass('mobile-native-select').addClass('select2');
            if (typeof __select2 === 'function') {
                __select2($('#price_group'));
            } else if (typeof $.fn.select2 !== 'undefined') {
                $('#price_group').select2();
            }
        } else if ($('#price_group').length) {
            // Mobile: Keep as native select with mobile styling
            $('#price_group').addClass('mobile-native-select').removeClass('select2');
        }
    });
}

// Initialize price group on page load
initPriceGroupSelect2();
...
$(document).ready(function() {
    ...
    dd_search_options = search_options = search_options.filter(option => option.id !== 23);


    $('#search_settings').select2({
        data: dd_search_options,
    ...
    $.ajax({
        ...
        success: function(result) {
            if (result) {
                ...
                toggle_dsp_input();
                $(document).trigger('mainFormPartLoaded');
