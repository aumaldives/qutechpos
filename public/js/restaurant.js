...
function refresh_orders() {
    ...
    // Choose the appropriate endpoint based on orders_for
    var refreshUrl = '/modules/refresh-orders-list';
    if (orders_for === 'waiter') {
        refreshUrl = '/modules/refresh-orders-list-waiter';
    }
    ...
    $.ajax({
        ...
        success: function(data) {
            // Fade out old content, update, then fade in new content
            $('#orders_div').fadeOut(150, function() {
            ...
            var orderCount = $('#orders_div .order_div').length;
            $('.badge.bg-blue, .badge.bg-green').text(orderCount);
        },
        error: function() {
            $('.overlay').addClass('hide');
            $refreshBtn.prop('disabled', false).html(originalText);
    ...
    // Only refresh line orders for kitchen view
    if (orders_for === 'kitchen') {
        $.ajax({
            ...
            dataType: 'html',
            success: function(data) {
                if ($('#line_orders_div').length) {
                    $('#line_orders_div').fadeOut(150, function() {
