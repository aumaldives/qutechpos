		...
		<div class="form-group">
			<div class="input-group">
				...
				<span class="input-group-btn">
					<button type="button" class="btn btn-default bg-white btn-flat add_new_customer" data-name=""  @if(!auth()->user()->can('customer.create')) disabled @endif><i class="fas fa-plus-circle fa-lg text-success"></i></button>
				</span>
			</div>
		...
		<div class="form-group">
			<div class="input-group">
				<div class="input-group-btn">
					<button type="button" class="btn btn-default bg-white btn-flat" data-toggle="modal" data-target="#configure_search_modal" title="{{__('lang_v1.configure_product_search')}}"><i class="fas fa-search-plus text-info"></i></button>
				</div>
				...
				<span class="input-group-btn">
					...
					@if(isset($pos_settings['enable_weighing_scale']) && $pos_settings['enable_weighing_scale'] == 1)
						...
						title="@lang('lang_v1.weighing_scale')"><i class="fas fa-balance-scale fa-lg text-warning"></i></button>
					@endif
					

					<button type="button" class="btn btn-default bg-white btn-flat pos_add_quick_product" data-href="{{action([\App\Http\Controllers\ProductController::class, 'quickAdd'])}}" data-container=".quick_add_product_modal"><i class="fas fa-plus-circle fa-lg text-success"></i></button>
				</span>
			</div>
		</div>
	</div>
...
<div class="row">
	...
	@if(!empty($price_groups) && count($price_groups) > 1)
		<div class="col-md-4 col-sm-6">
			<div class="form-group">
				<div class="input-group">
					<span class="input-group-addon">
						<i class="fad fa-money-bill-alt text-primary"></i>
					</span>
					@php
						reset($price_groups);
	...
	@if(in_array('types_of_service', $enabled_modules) && !empty($types_of_service))
		<div class="col-md-4 col-sm-6">
			<div class="form-group">
				<div class="input-group">
					<span class="input-group-addon">
						<i class="fad fa-external-link-square text-primary service_modal_btn"></i>
					</span>
	...
	@if(in_array('subscription', $enabled_modules))
		<div class="col-md-4 col-sm-6">
            ...
            </label><button type="button" data-toggle="modal" data-target="#recurringInvoiceModal" class="btn btn-link"><i class="fad fa-external-link-square text-primary"></i></button>@show_tooltip(__('lang_v1.recurring_invoice_help'))
		</div>
	@endif
    ...
    @endif
	<!--- added Purchase order No-->
	<div class="col-md-4 col-sm-6">
	<div class="form-group">
    {{-- !! Form::label('porder_no', __('lang_v1.purchase_order_no'), ['class' => 'control-label']) !! --}}
    {!! Form::text('porder_no', !empty($transaction) ? $transaction->porder_no : '', ['class' => 'form-control', 'placeholder' => __('lang_v1.purchase_order_no'), 'id' => 'porder_no']) !!}
</div>
	</div>
		...
		<table class="table table-condensed table-bordered table-striped table-responsive" id="pos_table">
			<thead>
				<tr>
					...
					@endif
					<th class="text-center col-md-2 {{$hide_tax_1}}">
						@lang('lang_v1.price_exc_tax')
					</th>
