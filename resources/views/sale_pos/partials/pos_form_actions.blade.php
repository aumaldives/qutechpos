...
<div class="row">
	<div class="pos-form-actions">
		<div class="col-md-12">
			...
			@endif
			<button type="button" class="@if($is_mobile) col-xs-6 @endif btn bg-info text-white btn-default btn-flat @if($pos_settings['disable_draft'] != 0) hide @endif" id="pos-draft" @if(!empty($only_payment)) disabled @endif><i class="fas fa-file-alt"></i> @lang('sale.draft')</button>
			<button type="button" class="btn btn-default bg-yellow btn-flat @if($is_mobile) col-xs-6 @endif" id="pos-quotation" @if(!empty($only_payment)) disabled @endif><i class="fas fa-file-invoice"></i> @lang('lang_v1.quotation')</button>
			...
			<button type="button" class="btn btn-success @if(!$is_mobile) @endif btn-flat no-print @if($pos_settings['disable_express_checkout'] != 0 || !array_key_exists('bank_transfer', $payment_types)) hide @endif pos-express-finalize @if($is_mobile) col-xs-6 @endif" data-pay_method="bank_transfer" title="@lang('tooltip.express_checkout')" style="background-color: #74348c; border:1px solid #74348c"><i class="fas fa-university"></i> Bank Transfer</button>
			<!-- Nabeel -->

			@if(empty($edit))
				<button type="button" class="btn btn-danger btn-flat @if($is_mobile) col-xs-6 @else btn-xs @endif" id="pos-cancel"> <i class="fas fa-times-circle"></i> @lang('sale.cancel')</button>
			@else
			...
			<button type="button" class="pull-right btn btn-primary btn-flat @if($is_mobile) col-xs-6 @endif" data-toggle="modal" data-target="#recent_transactions_modal" id="recent-transactions" style="margin-top:5px;"> <i class="fas fa-history"></i> @lang('lang_v1.recent_transactions')</button>
			@endif
...
@include('sale_pos.partials.edit_plasticbag_modal')