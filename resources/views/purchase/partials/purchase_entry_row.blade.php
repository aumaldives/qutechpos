    ...
    <tr @if(!empty($purchase_order_line)) data-purchase_order_id="{{$purchase_order_line->transaction_id}}" @endif @if(!empty($purchase_requisition_line)) data-purchase_requisition_id="{{$purchase_requisition_line->transaction_id}}" @endif>
        ...
        <td>
            ...
            @if($product->enable_sr_no == 1)
                <br>
                <button type="button" class="btn btn-xs btn-primary add_imei_numbers" data-row="{{$row_count}}" data-product="{{$product->name}}">
                    <i class="fa fa-barcode"></i> @lang('lang_v1.add_imei_numbers')
                </button>
                <div id="imei_container_{{$row_count}}" class="imei_container" style="margin-top: 5px;"></div>
                <input type="hidden" name="purchases[{{$row_count}}][imeis]" class="imei_input" value="">
            @endif
        </td>
        <td>
            @php
