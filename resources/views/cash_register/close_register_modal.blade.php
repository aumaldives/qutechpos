<div class="modal-dialog modal-lg" role="document">
  <div class="modal-content">
    ...
    <div class="modal-body">
        ...
        <!-- Debug: cash_adjustments isset: {{ isset($cash_adjustments) ? 'YES' : 'NO' }} -->
        @if(isset($cash_adjustments))
            <!-- Debug: cash_adjustments count: {{ $cash_adjustments->count() }} -->
            @if($cash_adjustments->count() > 0)
            <div class="row">
              <div class="col-md-12">
                <h4>@lang('cash_register.cash_adjustments')</h4>
              <table class="table table-bordered table-striped">
                <thead>
                  <tr>
                    <th>@lang('messages.date')</th>
                    <th>@lang('cash_register.adjustment_type')</th>
                    <th>@lang('sale.amount')</th>
                    <th>@lang('cash_register.adjustment_reason')</th>
                  </tr>
                </thead>
                <tbody>
                  @foreach($cash_adjustments as $adjustment)
                  <tr>
                    <td>{{ @format_datetime($adjustment->created_at) }}</td>
                    <td>
                      @if($adjustment->type == 'credit')
                        <span class="label label-success">@lang('cash_register.add_cash')</span>
                      @else
                        <span class="label label-danger">@lang('cash_register.remove_cash')</span>
                      @endif
                    </td>
                    <td>
                      @if($adjustment->type == 'credit')
                        <span class="text-green">+@format_currency($adjustment->amount)</span>
                      @else
                        <span class="text-red">-@format_currency($adjustment->amount)</span>
                      @endif
                    </td>
        ...
        <div class="col-sm-4">
          <div class="form-group">
            {!! Form::label('closing_amount', __( 'cash_register.total_cash' ) . ':*') !!}
              {!! Form::text('closing_amount', @num_format($register_details->cash_in_hand + $register_details->total_cash - $register_details->total_cash_refund - $register_details->total_cash_expense + ($register_details->total_cash_adjustments ?? 0)), ['class' => 'form-control input_number', 'required', 'placeholder' => __( 'cash_register.total_cash' ) ]); !!}
          </div>
        </div>
        <div class="col-sm-4">
          <div class="form-group">
