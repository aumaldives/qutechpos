@extends('layouts.app')
@section('title', __('Edit Plastic Bag Purchase'))

@section('content')
<!-- Content Header (Page header) -->
<section class="content-header">
    <h1>@lang('Edit Plastic Bag Purchase')</h1>
    <ol class="breadcrumb">
...
<section class="content">
    @component('components.widget', ['class' => 'box-primary', 'title' => __('Edit Plastic Bag Purchase')])
        {!! Form::model($purchase, ['url' => action([\App\Http\Controllers\PlasticBagController::class, 'updatePurchase'], [$purchase->id]), 'method' => 'put', 'id' => 'plastic_bag_purchase_edit_form', 'enctype' => 'multipart/form-data']) !!}
            ...
            <div class="col-sm-3">
                <div class="form-group">
                    ...
                    <div class="input-group">
                        ...
                        {!! Form::text('purchase_date', $purchase->purchase_date->format('d/m/Y'), ['class' => 'form-control', 'required', 'readonly']); !!}
                    </div>
                </div>
            </div>
            ...
            <div class="col-sm-3">
                <div class="form-group">
                    ...
                    @if($purchase->invoice_file)
                        <p class="text-muted">Current: <a href="{{ asset('storage/' . $purchase->invoice_file) }}" target="_blank">View File</a></p>
                    @endif
                </div>
            </div>
        </div>
            ...
            <div class="col-sm-12">
                ...
                <div class="table-responsive">
                    <table class="table table-condensed table-bordered table-striped" id="plastic_bag_purchase_table">
                        ...
                        <tbody>
                            @foreach($purchase->purchaseLines as $index => $line)
                            <tr class="purchase_line">
                                <td>
                                    {!! Form::select('plastic_bag_types['.$index.'][type_id]', $plastic_bag_types->pluck('name', 'id')->prepend('Please Select', ''), $line->plastic_bag_type_id, ['class' => 'form-control plastic_bag_type select2', 'required', 'style' => 'width:100%']) !!}
                                </td>
                                <td>
                                    {!! Form::text('plastic_bag_types['.$index.'][quantity]', $line->quantity, ['class' => 'form-control input_number purchase_quantity', 'required']); !!}
                                </td>
                                <td>
                                    {!! Form::text('plastic_bag_types['.$index.'][price_per_bag]', $line->price_per_bag, ['class' => 'form-control input_number price_per_bag', 'required']); !!}
                                </td>
                                <td>
                                    <span class="line_total">{{ number_format($line->line_total, 2) }}</span>
                                </td>
                                <td>
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <th>
                                    <span id="total_purchase_amount">{{ number_format($purchase->total_amount, 2) }}</span>
                                </th>
                                <th></th>
                            </tr>
                        </tfoot>
        ...
        <div class="row">
            <div class="col-sm-12">
                <button type="submit" class="btn btn-primary pull-right" id="submit_purchase_btn">@lang( 'messages.update' )</button>
            </div>
        </div>
...
@section('javascript')
    <script type="text/javascript">
        $(document).ready(function(){
            var purchase_line_index = {{ count($purchase->purchaseLines) }};
            ...
            $(document).on('submit', 'form#plastic_bag_purchase_edit_form', function(e) {
                e.preventDefault();
