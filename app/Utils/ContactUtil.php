...
class ContactUtil extends Util
{
    ...
    /**
     * Generate a unique contact ID for the business
     *
     * @param  int  $business_id
     * @return string
     */
    public function generateContactId($business_id)
    {
        // Get reference count for contacts
        $ref_count = $this->setAndGetReferenceCount('contacts', $business_id);
        
        // Generate reference number using existing method
        return $this->generateReferenceNumber('contacts', $ref_count, $business_id);
    }
     ...
     * @return array
     */
    public function getContactBalance($contact_id, $business_id)
    {
        ...
        // Calculate dues
        $total_purchase_due = ($contact->total_purchase ?? 0) - ($contact->purchase_paid ?? 0);
        $total_sell_due = ($contact->total_invoice ?? 0) - ($contact->invoice_received ?? 0);
        $opening_balance_due = ($contact->opening_balance ?? 0) - ($contact->opening_balance_paid ?? 0);
        $total_purchase_return_due = ($contact->total_purchase_return ?? 0) - ($contact->purchase_return_paid ?? 0);
        $total_sell_return_due = ($contact->total_sell_return ?? 0) - ($contact->sell_return_paid ?? 0);

        return [
            'total_purchase_due' => (float) $total_purchase_due,
            'total_sell_due' => (float) $total_sell_due,
            'opening_balance_due' => (float) $opening_balance_due,
            'total_purchase_return_due' => (float) $total_purchase_return_due,
            'total_sell_return_due' => (float) $total_sell_return_due,
            'last_transaction_date' => $contact->last_transaction_date
        ];
    }

    /**
     * Add opening balance for contact
     *
     * @param  int  $contact_id
     ...
     * @return bool
     */
    public function addOpeningBalance($contact_id, $opening_balance, $business_id)
    {
        ...
        $transactionUtil = new TransactionUtil();
        return $transactionUtil->createOpeningBalanceTransaction(
            $business_id, 
            $contact_id, 
            $opening_balance, 
            auth()->id() ?? 1, 
