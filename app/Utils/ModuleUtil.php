...
class ModuleUtil extends Util
{
    ...
    public function getModuleData($function_name, $arguments = null)
    {
        ...
        // Check for core application permissions first
        $core_class = 'App\Http\Controllers\DataController';
        if (class_exists($core_class)) {
            $class_object = new $core_class();
            if (method_exists($class_object, $function_name)) {
                if (! empty($arguments)) {
                    $data['Core'] = call_user_func([$class_object, $function_name], $arguments);
                } else {
                    $data['Core'] = call_user_func([$class_object, $function_name]);
                }
            }
        }
    ...
    public function isQuotaAvailable($type, $business_id, $total_rows = 0)
    {
        ...
        if ($is_available) {
            ...
            if ($type == 'locations') {
                ...
                // Use getEffectiveLocationCount() for per-location subscriptions
                $max_allowed = $package->getEffectiveLocationCount();
                if ($max_allowed == 0) {
                    return true;
                } else {
