...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        // Use invoice_total calculation, not input final_total to avoid double-counting plastic bags
        ...
        $transaction = Transaction::create([
            ...
            'plasticbag_quantity' => !empty($input['plasticbag_quantity']) ? $input['plasticbag_quantity'] : 0,
            'plasticbag_charges' => !empty($input['plasticbag_charges']) ? $input['plasticbag_charges'] : 0,
            'porder_no' => !empty($input['porder_no']) ? $input['porder_no'] : 0,
    ...
    public function createOrUpdateSellLines($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        ...
        if (!empty($lines_formatted)) {
            ...
            // Process IMEI data for sold products
            $this->processImeiForSellLines($transaction, $products, $lines_formatted);
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if ($il->show_letter_head == 1) {
            ...
            $output['letter_head'] = ($il->letter_head) ? asset('uploads/invoice_logos/' . $il->letter_head) : null;
        }
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            foreach ($details['lines'] as $line) {
                ...
                if (1 == 1) {
                    $c_quantity =  $line['quantity'];
                    $unitQuantity = $line['base_unit_multiplier'];
                }

                if ($line['item_tax'] != 0 && $line['item_tax'] != "") {
                    $total_tax += $line['item_tax'];
                }
        ...
        //Plasticbag
        $output['plasticbag_charges'] = ($transaction->plasticbag_charges != 0) ? $this->num_f($transaction->plasticbag_charges, $show_currency, $business_details) : 0;
        $output['plasticbag_charges_label'] = trans('sale.plasticbag_charges');
        
        // Detailed plastic bag usage breakdown
        $output['plastic_bag_details'] = [];
        if ($transaction->plasticbag_charges > 0) {
            try {
                $plastic_bag_usage = \App\PlasticBagUsage::where('transaction_id', $transaction->id)
                    ->with('plasticBagType')
                    ->get();
                
                foreach ($plastic_bag_usage as $usage) {
                    if ($usage->plasticBagType) {
                        $output['plastic_bag_details'][] = [
                            'type' => $usage->plasticBagType->name,
                            'quantity' => $usage->quantity,
                            'price' => $this->num_f($usage->selling_price, $show_currency, $business_details),
                            'total' => $this->num_f($usage->quantity * $usage->selling_price, $show_currency, $business_details)
                        ];
                    }
                }
            } catch (\Exception $e) {
                // Log the error but don't break the receipt generation
                \Log::error('Error loading plastic bag usage details for transaction ' . $transaction->id, [
                    'error' => $e->getMessage(),
        ...
        $output['porder_no'] = ($transaction->porder_no !== null) ? $transaction->porder_no : null;
        $output['porder_no_label'] = trans('sale.porder_no');
        ...
        if ($transaction_type == 'sell' && $transaction->status == 'final') {
            ...
            // final_total already includes plastic bag charges, so use it directly
            ...
            // Always show total_due if there's any due amount, even if zero
            if ($due > 0) {
                $output['total_due'] = $this->num_f($due, $show_currency, $business_details);
        ...
        $output['hide_invoice_tax'] = $il->hide_invoice_tax;
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            $line_array['item_tax'] = !empty($line->item_tax) ? round(nl2br($line->item_tax * $line->quantity), 2) : '';
            //$tax_total+=round($line->item_tax,2);
    ...
    public function calculateRewardPoints($business_id, $total)
    {
        if (session()->has('business')) {
            ...
            // Convert to object if it's an array from session
        ...
        // Handle both array and object access for business data
        $enable_rp = is_array($business) ? $business['enable_rp'] ?? 0 : ($business->enable_rp ?? 0);
        
        if ($enable_rp == 1) {
            //check if order total elegible for reward
            $min_order_total = is_array($business) ? $business['min_order_total_for_rp'] ?? 0 : ($business->min_order_total_for_rp ?? 0);
            if ($min_order_total > $total) {
                return $total_points;
            }
            
            $amount_per_unit_point = is_array($business) ? $business['amount_for_unit_rp'] ?? 1 : ($business->amount_for_unit_rp ?? 1);
            $total_points = floor($total / $amount_per_unit_point);

            $max_rp_per_order = is_array($business) ? $business['max_rp_per_order'] ?? null : ($business->max_rp_per_order ?? null);
            if (!empty($max_rp_per_order) && $max_rp_per_order < $total_points) {
                $total_points = $max_rp_per_order;
            }
        }
    ...
    public function deleteSale($business_id, $transaction_id)
    {
        ...
        if (!empty($transaction)) {
            ...
            if ($transaction->status == 'draft') {
                ...
                //Delete public payment invoice mappings first to avoid foreign key constraint violation
                \DB::table('public_payment_invoice_mappings')->where('transaction_id', $transaction->id)->delete();
                ...
                //Fire payment deleted events before deleting the transaction
    ...
    public function getLedgerDetails($contact_id, $start, $end, $format = 'format_1', $location_id = null, $line_details = false)
    {
        ...
        foreach ($transactions as $transaction) {
            ...
            if ($format == 'format_2') {
                // final_total already includes plastic bag charges
    ...
    public function getEmailAttachmentForGivenTransaction($business_id, $transaction_id, $is_email_attachment)
    {
        ...
        // Get the correct template based on invoice layout design
        ...
        } elseif (!empty($invoice_layout->design)) {
            $blade_file = $invoice_layout->design;
        }

        //Generate pdf
        $body = view('sale_pos.receipts.' . $blade_file)
            ->with(compact('receipt_details', 'location_details', 'is_email_attachment', 'invoice_layout'))
            ->render();
    ...
    public function getOutputMiraFirstSheet($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
        $query = Transaction::where('transactions.business_id', $business_id)
            ->join('contacts as ct', 'ct.id', '=', 'transactions.contact_id')
            ...
            ->where('transactions.status', "<>", 'draft')
            ->where('ct.tax_number', "<>", '')
            ->groupBy('transactions.invoice_no')
            ->select(
                "ct.tax_number",
                DB::raw('CASE WHEN ct.supplier_business_name IS NOT NULL THEN ct.supplier_business_name ELSE ct.name END AS name'),
                "transactions.invoice_no",
                "transactions.transaction_date",
                "bl.activity_number",
                //DB::raw('SUM(CASE WHEN tsl.tax_id IS NOT NULL THEN tsl.unit_price * tsl.quantity ELSE 0 END) AS total_tax_applicable'),
                DB::raw('SUM(CASE WHEN tsl.tax_id IS NOT NULL AND tsl.item_tax > 0 THEN tsl.unit_price * tsl.quantity ELSE 0 END) AS total_tax_applicable'),
                //DB::raw('SUM(CASE WHEN tsl.tax_id IS NULL THEN tsl.unit_price * tsl.quantity ELSE 0 END) AS total_non_tax_applicable')
                DB::raw('SUM(CASE WHEN tsl.item_tax = 0 THEN tsl.unit_price * tsl.quantity ELSE 0 END) AS total_non_tax_applicable')
            );
        ...
        $output_data = $query->get();
        return $output_data;
    }

    public function getOutputMiraSecondSheet($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
        $query = Transaction::where('transactions.business_id', $business_id)
            ->join('contacts as ct', 'ct.id', '=', 'transactions.contact_id')
            ->join('transaction_sell_lines as tsl', 'tsl.transaction_id', '=', 'transactions.id')
            ...
            ->where('transactions.status', "<>", 'draft')
            ->whereNull('ct.tax_number')
            ->groupBy('bl.activity_number')
            ->select(
                "bl.activity_number",
                //DB::raw('SUM(CASE WHEN tsl.tax_id IS NOT NULL THEN tsl.unit_price * tsl.quantity ELSE 0 END) AS total_tax_applicable'),
                DB::raw('SUM(CASE WHEN tsl.tax_id IS NOT NULL AND tsl.item_tax > 0 THEN tsl.unit_price * tsl.quantity ELSE 0 END) AS total_tax_applicable'),
    ...
    public function getInputMira($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
        $query = Transaction::where('transactions.business_id', $business_id)
            ->join('contacts as ct', 'ct.id', '=', 'transactions.contact_id')
            ...
            ->select(
                ...
                "transactions.transaction_date",
                "bl.activity_number",
                DB::raw('SUM(pl.purchase_price * pl.quantity) AS total_tax_applicable'),
                DB::raw('SUM(CASE WHEN pl.tax_id IS NOT NULL AND pl.item_tax > 0 THEN pl.item_tax * pl.quantity ELSE 0 END) AS item_tax_amount')
    ...
    /**
     * Check and apply auto-cook logic for restaurant orders
     *
     * @param int $transaction_id
     * @param int $business_id
     * @return bool Whether auto-cook was applied
     */
    public function checkAndApplyAutoCook($transaction_id, $business_id)
    {
        try {
            // Get auto-cook categories for this business
            $auto_cook_categories = \App\KitchenSetting::getAutoCookCategories($business_id);
            
            if (empty($auto_cook_categories)) {
                return false;
            }

            // Get the transaction with sell lines and product details
            $transaction = Transaction::with(['sell_lines.product.category'])
                                    ->find($transaction_id);
            ...
            // Check if all items in the order belong to auto-cook categories
            $all_items_auto_cook = true;
            
            foreach ($transaction->sell_lines as $line) {
                if (!$line->product) {
                    $all_items_auto_cook = false;
                    break;
                }
                ...
                // Check if this product's category is in auto-cook list
                $is_auto_cook_category = in_array($product_category_id, $auto_cook_categories);
                
                // If not found, check parent category
                if (!$is_auto_cook_category && $line->product->category && $line->product->category->parent_id) {
                    $is_auto_cook_category = in_array($line->product->category->parent_id, $auto_cook_categories);
                }
                
                if (!$is_auto_cook_category) {
                    $all_items_auto_cook = false;
                    break;
                }
            }

            // If all items belong to auto-cook categories, mark them as cooked
            if ($all_items_auto_cook) {
                TransactionSellLine::where('transaction_id', $transaction_id)
                                  ->where(function($query) {
                                      $query->whereNull('res_line_order_status')
                                            ->orWhere('res_line_order_status', '')
            ...
            \Log::error('Auto-cook check failed: ' . $e->getMessage());
            return false;
        }
    }

    /**
     * Process IMEI data for sell lines
     */
    private function processImeiForSellLines($transaction, $products, $lines_formatted)
    {
        // Create mapping for easy lookup
        $sell_line_mapping = [];
        foreach ($lines_formatted as $sell_line) {
            $key = $sell_line->product_id . '_' . $sell_line->variation_id;
            $sell_line_mapping[$key] = $sell_line;
        }

        foreach ($products as $product_data) {
            if (empty($product_data['selected_imeis'])) {
                continue;
            }

            $key = $product_data['product_id'] . '_' . $product_data['variation_id'];
            if (!isset($sell_line_mapping[$key])) {
                continue;
            }

            $sell_line = $sell_line_mapping[$key];
            $selected_imei_ids = explode(',', trim($product_data['selected_imeis']));
            
            foreach ($selected_imei_ids as $imei_id) {
                if (empty(trim($imei_id))) {
                    continue;
                }
                
                $imei_record = ProductImei::find($imei_id);
                if ($imei_record) {
                    $imei_record->update([
                        'is_sold' => 1,
