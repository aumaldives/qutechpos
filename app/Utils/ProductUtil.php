...
class ProductUtil extends Util
{
    ...
    public function getVariationGroupPrice($variation_id, $price_group_id, $tax_id)
    {
        ...
        // Handle case where price group is not found
        if (!$price_group) {
            \Log::warning('VariationGroupPrice not found', [
                'variation_id' => $variation_id,
                'price_group_id' => $price_group_id
            ]);
            
            // Fallback to variation's default sell price
            $variation = Variation::find($variation_id);
            $price_inc_tax = $variation ? $variation->sell_price_inc_tax : 0;
        } elseif(isset($price_group->price_type) && $price_group->price_type == 'percentage'){
            //calculate the price
            $variation = Variation::find($variation_id);
    ...
    public function createOrUpdatePurchaseLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
        ...
        if (! empty($updated_purchase_lines)) {
            ...
            // Process IMEI data for each purchase line
            $this->processImeiForPurchaseLines($transaction, $input_data, $updated_purchase_lines);
        }
            ...
            if ($search_type == 'like') {
                $query->where(function ($query) use ($search_term, $search_fields) {
                    ...
                    if (in_array('barcode', $search_fields)) {
                        $query->orWhere('barcode', 'like', '%'.$search_term.'%');
                    }
    ...
    /**
     * Process IMEI data for purchase lines
     *
     * @param Transaction $transaction
     * @param array $input_data
     * @param array $updated_purchase_lines
     * @return void
     */
    public function processImeiForPurchaseLines($transaction, $input_data, $updated_purchase_lines)
    {
        // Index purchase lines by their product/variation for easy lookup
        $purchase_line_mapping = [];
        foreach ($updated_purchase_lines as $index => $purchase_line) {
            $key = $purchase_line->product_id . '_' . $purchase_line->variation_id;
            $purchase_line_mapping[$key] = $purchase_line;
        }

        foreach ($input_data as $data) {
            // Check if IMEI data exists for this purchase line
            if (empty($data['imeis'])) {
                continue;
            }

            $key = $data['product_id'] . '_' . $data['variation_id'];
            if (!isset($purchase_line_mapping[$key])) {
                continue;
            }

            $purchase_line = $purchase_line_mapping[$key];
            $imei_string = trim($data['imeis']);
            
            if (empty($imei_string)) {
                continue;
            }

            // Split comma-separated IMEI values
            $imei_array = array_filter(array_map('trim', explode(',', $imei_string)));
            
            if (empty($imei_array)) {
                continue;
            }

            // Clear existing IMEIs for this purchase line (in case of updates)
            ProductImei::where('purchase_line_id', $purchase_line->id)->delete();

            // Create new IMEI records
            foreach ($imei_array as $imei) {
                if (empty($imei)) {
                    continue;
                }

                // Check if IMEI already exists in this business
                if (!ProductImei::isImeiUnique($imei, $transaction->business_id)) {
                    // Log warning but continue processing other IMEIs
                    \Log::warning('Duplicate IMEI found in purchase', [
                        'imei' => $imei,
