...
class Util
{
    ...
    public function allModulesEnabled($business_id = null)
    {
        ...
        if (!is_array($enabled_modules) || $enabled_modules === 'null') {
            $enabled_modules = [];
        }
    ...
    private function sendSmsViaMsgOwl($data)
    {
        ...
        try {
            $msgowl = new \App\Utils\MsgOwlUtil(
    ...
    public function sendSms($data)
    {
        ...
        if ($sms_service == 'msgowl') {
            return $this->sendSmsViaMsgOwl($data);
        }
     ...
     * @return array
     */
    public function getSubUnits2($business_id, $unit_id, $return_main_unit_if_empty = false, $product_id = null)
    {
    ...
    public function getSubUnits($business_id, $unit_id, $return_main_unit_if_empty = false, $product_id = null, $sub_unit_prices = null, $default_price = null)
    {
        $unit = Unit::where('business_id', $business_id)
            ->with(['sub_units'])
            ->findOrFail($unit_id);
        ...
        if (count($unit->sub_units) > 0) {
            foreach ($unit->sub_units as $sub_unit) {
                ...
                if (empty($related_sub_units) || in_array($sub_unit->id, $related_sub_units)) {
                    ...
                    if ($sub_unit_prices) {
                        $unitPrice = $sub_unit_prices->firstWhere('sub_unit_id', $sub_unit->id);
                        if ($unitPrice) {
                            //$multiplier = $unitPrice->default_sell_price; 
                            $multiplier = $multiplier; 
                            $custom_unit_price = $unitPrice->default_sell_price / $default_price; 
                            $default_sell_price = $unitPrice->default_sell_price;
                            $custom_unit = 1;
                        }
                    }
    ...
    public function uploadFile($request, $file_name, $dir_name, $file_type = 'document')
    {
        ...
        $mainFolder = config('constants.mainFolder');
        ...
        if ($request->hasFile($file_name) && $request->file($file_name)->isValid()) {
            ...
            if ($request->$file_name->getSize() <= config('constants.document_size_limit')) {
                ...
                if ($request->$file_name->storeAs($mainFolder.'/'.$dir_name, $new_file_name)) {
    ...
    public function replaceTags($business_id, $data, $transaction, $contact = null)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if (strpos($value, '{total_amount}') !== false) {
                // Include plastic bag charges in total amount
                $actual_total = $transaction->final_total + ($transaction->plasticbag_charges ?? 0);
                $total_amount = $this->num_f($actual_total, true, $business->currency);
            ...
            if (strpos($value, '{due_amount}') !== false) {
                // Include plastic bag charges in due amount calculation
                $actual_total = $transaction->final_total + ($transaction->plasticbag_charges ?? 0);
                $due = $actual_total - $total_paid;
    ...
    public function activityLog($on, $action = null, $before = null, $properties = [], $log_changes = true, $business_id = null)
    {
        ...
        $time_zone = is_array($business) ? $business['time_zone'] ?? 'UTC' : ($business->time_zone ?? 'UTC');
