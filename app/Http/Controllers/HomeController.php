...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($module_widgets as $module_name => $module_widget_array) {
            if (is_array($module_widget_array)) {
                foreach ($module_widget_array as $widget_array) {
    ...
    public function integrations(){
        ...
        // Get user permissions for integration features
        $user_permissions = [
            'can_manage_api_keys' => auth()->user()->can('manage_api_keys'),
            'can_view_api_docs' => auth()->user()->can('view_api_docs'),
        ];
        
        // Check module availability through subscription system
        $quickbooks_enabled = false;
        $quickbooks_required_package = null;
        $quickbooks_upgrade_available = false;
        
        if (auth()->user()->can('superadmin')) {
            // Superadmins can access all modules if they exist and are enabled
            $quickbooks_enabled = \Module::find('Quickbooks') && \Module::find('Quickbooks')->isEnabled();
        } else {
            // Regular users need subscription permission
            $quickbooks_enabled = $this->moduleUtil->hasThePermissionInSubscription($business_id, 'quickbooks_module', 'superadmin_package');
            
            // If not enabled, check if QuickBooks is available in any publicly accessible package
            if (!$quickbooks_enabled) {
                $quickbooks_required_package = $this->getPackageWithQuickBooks();
                $quickbooks_upgrade_available = $quickbooks_required_package !== null;
            }
        }
        
        $module_availability = [
            'quickbooks_enabled' => $quickbooks_enabled,
            'quickbooks_required_package' => $quickbooks_required_package,
            'quickbooks_upgrade_available' => $quickbooks_upgrade_available,
        ];
    ...
    /**
     * Get the package name that includes QuickBooks integration and is available on public pricing page
     */
    private function getPackageWithQuickBooks()
    {
        // Check if superadmin module is installed to get package information
        if ($this->moduleUtil->isSuperadminInstalled()) {
            try {
                // Get packages that include quickbooks_module, are active, publicly available (not private),
                // and not restricted by any date restrictions (same logic as public pricing page)
                $package = \Modules\Superadmin\Entities\Package::where('custom_permissions->quickbooks_module', '1')
                                                               ->where('is_active', 1)
                                                               ->where('is_private', 0) // Exclude private packages
                                                               ->whereNull('restrict_date') // Exclude "Restrict - Signup Before Date" packages
                                                               ->whereNull('restrict_date_after') // Exclude "Restrict - Signup After Date" packages
                                                               ->orderBy('sort_order')
                                                               ->first();
                ...
                // If no publicly available packages have QuickBooks, return null to indicate it's not available for upgrade
                return null;
            } catch (\Exception $e) {
                // Fallback if there's an issue with database query
                return null;
            }
        }
        
        return null; // If superadmin module not installed, QuickBooks upgrade not available
    }
}
