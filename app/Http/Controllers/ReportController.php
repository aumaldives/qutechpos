...
use App\Exports\TaxInvoicesExport;
use App\Exports\MiraTaxReportExport;
use Maatwebsite\Excel\Facades\Excel;
use Illuminate\Support\Facades\Crypt;
use Illuminate\Support\Facades\Storage;
use Spatie\Activitylog\Models\Activity;
use App\Exports\OtherTransactionsExport;
...
class ReportController extends Controller
{
    ...
    public function getStockReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            $filters = request()->only([
                ...
                'only_mfg_products', 'active_state',  'not_for_selling', 'repair_model_id', 'product_id', 'active_state','supplier_business_name',
            ]);
            ...
            $datatable = Datatables::of($products)
                ...
                ->editColumn('product', function ($row) {
                 ...
                 ->addColumn('supplier_business_name', function ($row) {
                        $supplier_business_name = DB::select(DB::raw('
                        SELECT a.supplier_business_name 
                        FROM contacts AS a
                        LEFT JOIN transactions AS b ON a.id = b.contact_id
                        LEFT JOIN purchase_lines AS c ON c.transaction_id = b.id
                        WHERE c.product_id = ?
                        ORDER BY c.id DESC
                        LIMIT 1
                    '), [$row->product_id]);

                    if (!empty($supplier_business_name)) {
                        return $supplier_business_name[0]->supplier_business_name;
                    }
                    return '-';
    ...
    public function getMiraTaxReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $fileName = Crypt::encryptString(Str::random(6) . '_' . $timestamp) . '.xlsx';
            $path = 'uploads/mira_output_tax_statement/';

            $output_tax_data = $this->transactionUtil->getOutputMiraFirstSheet($business_id, $start_date, $end_date, $location_id, $contact_id);

            $output_tax_data_second_sheet = $this->transactionUtil->getOutputMiraSecondSheet($business_id, $start_date, $end_date, $location_id, $contact_id);
            
            $firstSheetDataHolder = array();
            $secondSheetDataHolder = array();

            if ($output_tax_data->isEmpty()) {
                //echo "No record found";
            } else {
                $firstSheetData = $output_tax_data->toArray();
                 foreach($firstSheetData as $row){
                    $rowData = [
                        ...
                        $row['total_tax_applicable'],
                        $row['total_non_tax_applicable'],
                        "",
                        "",
                        $row['activity_number'],
                    ];
                    array_push($firstSheetDataHolder, $rowData);
                }

                //dd($firstSheetDataHolder);


            }

            if ($output_tax_data_second_sheet->isEmpty()) {
                //echo "No record found";
            } else {
                $secondSheetData = $output_tax_data_second_sheet->toArray();
                foreach($secondSheetData as $row){
                    ...
                    array_push($secondSheetDataHolder, $rowData);
                }
            }
            
            Excel::store(new class($firstSheetDataHolder, $secondSheetDataHolder) implements WithMultipleSheets
            {
                private $firstSheetData;
                private $secondSheetData;
            
                public function __construct($firstSheetData, $secondSheetData){
                    $this->firstSheetData = $firstSheetData;
                    $this->secondSheetData = $secondSheetData;
                }

                public function sheets(): array{
                     return [
                         'Tax Invoices' => new TaxInvoicesExport($this->firstSheetData),
                         'Other Transactions' => new OtherTransactionsExport($this->secondSheetData),
                     ];
                }
            }, $path . $fileName);
    ...
    public function getMiraInputTaxReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $timestamp = now()->timestamp;
            $fileName = Crypt::encryptString(Str::random(6) . '_' . $timestamp) . '.xlsx';
            $path = 'uploads/mira_input_tax_statement/';

            $output_tax_data = $this->transactionUtil->getInputMira($business_id, $start_date, $end_date, $location_id, $contact_id);
            ...
            $firstSheetData = $output_tax_data->toArray();
                 foreach($firstSheetData as $key=>$row){
            ...
            Excel::store(new class($firstSheetDataHolder) implements WithMultipleSheets
            {
                private $firstSheetData;
            
                public function __construct($firstSheetData){
                    $this->firstSheetData = $firstSheetData;
                }

                public function sheets(): array{
                     return [
                         'Tax Invoices' => new InputExport($this->firstSheetData),
                     ];
                }
            }, $path . $fileName);
        ...
        if ($request->ajax()) {
            $registers = CashRegister::leftjoin(
                ...
                ->select(
                    ...
                    DB::raw("SUM(IF(transaction_type='adjustment', IF(type='credit', amount, -amount), 0)) as total_cash_adjustments"),
            ...
            return Datatables::of($registers)
                ...
                ->editColumn('total_cash_payment', function ($row) {
                    $total_cash = ($row->cash_in_hand ?? 0) + $row->total_cash_payment + ($row->total_cash_adjustments ?? 0);
                    return '<span data-orig-value="' . $total_cash . '" >' . $this->transactionUtil->num_f($total_cash, true) . '</span>';
                })
                ...
                ->addColumn('total', function ($row) {
                    $total = ($row->cash_in_hand ?? 0) + $row->total_card_payment + $row->total_cheque_payment + $row->total_cash_payment + $row->total_bank_transfer_payment + $row->total_other_payment + $row->total_advance_payment + $row->total_custom_pay_1 + $row->total_custom_pay_2 + $row->total_custom_pay_3 + $row->total_custom_pay_4 + $row->total_custom_pay_5 + $row->total_custom_pay_6 + $row->total_custom_pay_7 + ($row->total_cash_adjustments ?? 0);
    ...
    /**
     * Shows plastic bag report
     *
     * @return \Illuminate\Http\Response
     */
    public function getPlasticBagReport(Request $request)
    {
        if (!auth()->user()->can('plastic_bag_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($request->ajax() || $request->has('export')) {
            ...
            // Get plastic bag sales data grouped by bag type and location
            $sales_query = DB::table('plastic_bag_usage as pbu')
                ->join('plastic_bag_types as pbt', 'pbu.plastic_bag_type_id', '=', 'pbt.id')
                ->leftJoin('business_locations as bl', 'pbu.location_id', '=', 'bl.id')
                ->where('pbu.business_id', $business_id)
                ->whereBetween('pbu.usage_date', [$start_date, $end_date]);

            if (!empty($location_id)) {
                $sales_query->where('pbu.location_id', $location_id);
            }

            $sales_data = $sales_query->selectRaw('
                pbt.name as bag_type,
                bl.name as location,
                SUM(pbu.quantity) as total_bags_sold,
                SUM(pbu.quantity * pbu.selling_price) as total_revenue,
                AVG(pbu.selling_price) as avg_price_per_bag,
                COUNT(DISTINCT pbu.transaction_id) as total_transactions
            ')->groupBy('pbt.id', 'bl.id')->get();

            // Create location summary by aggregating all bag types per location
            $location_summary = $sales_data->groupBy('location')->map(function($location_data, $location_name) {
                $location_name = $location_name ?? 'Default';
                $bag_types = $location_data->pluck('bag_type')->unique()->implode(', ');
                
                return [
                    'location' => $location_name,
                    'bag_types' => $bag_types,
                    'total_bags' => $location_data->sum('total_bags_sold'),
                    'total_charges' => $location_data->sum('total_revenue'),
                    'transaction_count' => $location_data->sum('total_transactions')
                ];
            })->values();

            // Get purchase data
            $purchases_query = DB::table('plastic_bag_purchase_lines as pbpl')
                ->join('plastic_bag_purchases as pbp', 'pbpl.plastic_bag_purchase_id', '=', 'pbp.id')
                ->join('plastic_bag_types as pbt', 'pbpl.plastic_bag_type_id', '=', 'pbt.id')
                ->where('pbp.business_id', $business_id)
                ->whereBetween('pbp.purchase_date', [$start_date, $end_date]);

            $purchases_data = $purchases_query->selectRaw('
                pbt.name as bag_type,
                SUM(pbpl.quantity) as total_bags_purchased,
                SUM(pbpl.line_total) as total_purchase_amount,
                AVG(pbpl.price_per_bag) as avg_purchase_price
            ')->groupBy('pbt.id')->get();

            // Get stock adjustments
            $adjustments_query = DB::table('plastic_bag_stock_adjustments as pbsa')
                ->join('plastic_bag_types as pbt', 'pbsa.plastic_bag_type_id', '=', 'pbt.id')
                ->where('pbsa.business_id', $business_id)
                ->whereBetween('pbsa.adjustment_date', [$start_date, $end_date]);

            $adjustments_data = $adjustments_query->selectRaw('
                pbt.name as bag_type,
                pbsa.adjustment_type,
                pbsa.reason,
                SUM(pbsa.quantity) as total_adjusted,
                COUNT(*) as adjustment_count
            ')->groupBy('pbt.id', 'pbsa.adjustment_type', 'pbsa.reason')->get();

            // Get detailed transaction data for the table with pagination
            $transactions_base_query = DB::table('plastic_bag_usage as pbu')
                ->join('transactions as t', 'pbu.transaction_id', '=', 't.id')
                ->join('plastic_bag_types as pbt', 'pbu.plastic_bag_type_id', '=', 'pbt.id')
                ->leftJoin('business_locations as bl', 'pbu.location_id', '=', 'bl.id')
                ->where('pbu.business_id', $business_id)
                ->whereBetween('pbu.usage_date', [$start_date, $end_date]);
            ...
            if (!empty($location_id)) {
                $transactions_base_query->where('pbu.location_id', $location_id);
            }

            // If export is requested, get all data
            if ($export) {
                $transactions_data = $transactions_base_query->selectRaw('
                    t.transaction_date as date,
                    t.invoice_no,
                    COALESCE(bl.name, "Default") as location,
                    GROUP_CONCAT(pbt.name) as bag_types,
                    SUM(pbu.quantity) as bags,
                    SUM(pbu.quantity * pbu.selling_price) as charges
                ')->groupBy('t.id')->orderBy('t.transaction_date', 'desc')->get();
            } else {
                ...
                $total_transactions = $transactions_base_query
                    ->selectRaw('COUNT(DISTINCT t.id) as total')
                    ->first()->total;
            }

            // Calculate totals
            $total_bags = $location_summary->sum('total_bags');
            $total_charges = $location_summary->sum('total_charges');
            $transaction_count = $location_summary->sum('transaction_count');

            // Handle export requests
            if ($export === 'excel') {
                return $this->exportPlasticBagReportExcel($transactions_data, $location_summary, $start_date, $end_date);
            } elseif ($export === 'pdf') {
                return $this->exportPlasticBagReportPDF($transactions_data, $location_summary, $start_date, $end_date);
            }
            ...
            $report_data = [
                ...
                'location_summary' => $location_summary->toArray(),
                'sales' => $sales_data,
                'purchases' => $purchases_data,
                'adjustments' => $adjustments_data
            ];

            // Add pagination info for non-export requests
            if (!$export) {
                $report_data['pagination'] = [
                    'current_page' => $page,
                    'per_page' => $per_page,
                    'total' => $total_transactions ?? $transactions_data->count(),
                    'last_page' => ceil(($total_transactions ?? $transactions_data->count()) / $per_page)
                ];
            }
        ...
        return view('report.plastic_bag_report')->with(compact('business_locations'));
    }

    /**
     * Export plastic bag report to Excel (CSV format)
     */
    private function exportPlasticBagReportExcel($transactions, $location_summary, $start_date, $end_date)
    {
        $filename = 'plastic_bag_report_' . $start_date . '_to_' . $end_date . '.csv';
        ...
        $callback = function() use ($transactions, $location_summary, $start_date, $end_date) {
            ...
            // Report header
            fputcsv($file, ['Plastic Bag Report']);
            fputcsv($file, ['Period: ' . $start_date . ' to ' . $end_date]);
            fputcsv($file, []);
            
            // Location Summary
            fputcsv($file, ['Location Summary']);
            fputcsv($file, ['Location', 'Bag Types', 'Total Bags', 'Total Charges', 'Transaction Count']);
            foreach ($location_summary as $location) {
                fputcsv($file, [
                    $location['location'],
                    $location['bag_types'] ?? '',
                    $location['total_bags'],
                    number_format($location['total_charges'], 2),
            ...
            fputcsv($file, ['Date', 'Invoice No', 'Location', 'Bag Types', 'Bags Sold', 'Charges']);
            foreach ($transactions as $transaction) {
                fputcsv($file, [
                    $transaction->date,
                    $transaction->invoice_no,
                    $transaction->location,
                    $transaction->bag_types ?? '',
                    $transaction->bags,
    ...
    /**
     * Export plastic bag report to PDF
     */
    private function exportPlasticBagReportPDF($transactions, $location_summary, $start_date, $end_date)
    {
        // Convert collections to arrays if needed
        $location_summary_array = is_array($location_summary) ? $location_summary : $location_summary->toArray();
        $transactions_array = is_array($transactions) ? $transactions : $transactions->toArray();
        
        // Calculate totals
        $total_bags = collect($location_summary_array)->sum('total_bags');
        $total_charges = collect($location_summary_array)->sum('total_charges');
        $transaction_count = collect($location_summary_array)->sum('transaction_count');
        
        $data = [
            'transactions' => $transactions_array,
            'location_summary' => $location_summary_array,
        ...
        $pdf->loadView('report.plastic_bag_report_pdf', $data);
        
        $filename = 'plastic_bag_report_' . $start_date . '_to_' . $end_date . '.pdf';
