        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            if (auth()->user()->can('product.view') || auth()->user()->can('product.create') ||
                ...
                $menu->dropdown(
                    ...
                    ['icon' => 'fa fas fa-boxes', 'id' => 'tour_step5']
                )->order(20);
            }
            ...
            if (in_array('purchases', $enabled_modules) && (auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create') || auth()->user()->can('purchase.update'))) {
                $menu->dropdown(
                    ...
                    ['icon' => 'fa fas fa-cart-arrow-down', 'id' => 'tour_step6']
                )->order(25);
            }
            //Sell dropdown
            if ($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping', 'access_sell_return', 'direct_sell.view', 'direct_sell.update', 'access_own_sell_return'])) {
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin, $pos_settings) {
                        ...
                        // Warranty Management - Show if warranty is enabled (either product warranty or warranty management)
                        if ((!empty($pos_settings['enable_warranty_management']) || !empty($common_settings['enable_product_warranty'])) && auth()->user()->can('sell.view')) {
                            $sub->url(
                                action([\App\Http\Controllers\WarrantyManagementController::class, 'index']),
                                __('lang_v1.warranty_management'),
                                ['icon' => 'fa fas fa-shield', 'active' => request()->segment(1) == 'warranty-management']
                            );
                        }
                        
                        // IMEI Sales Search - Show for users who can view sales
                        if (auth()->user()->can('sell.view')) {
                            $sub->url(
                                action([\App\Http\Controllers\ImeiSalesSearchController::class, 'index']),
                                __('lang_v1.imei_sales_search'),
                                ['icon' => 'fa fas fa-mobile-alt', 'active' => request()->segment(1) == 'imei-sales-search']
                            );
                        }
                    },
                    ['icon' => 'fa fas fa-cash-register', 'id' => 'tour_step7']
                )->order(30);
            }

            //Pending Bank Payments (Bank Transfer Payments)
            if (auth()->user()->can('access_pending_payments') || auth()->user()->can('approve_pending_payments')) {
                // Count single invoice payments
                $single_pending_count = \DB::table('pending_bank_payments')
                    ->where('business_id', session('user.business_id'))
                    ->where('status', 'pending')
                    ->count();
                
                // Count multi-invoice payments from public payment portal
                $multi_pending_count = \DB::table('public_payment_submissions')
                    ->where('business_id', session('user.business_id'))
                    ->where('status', 'pending')
                    ->count();
                
                $pending_count = $single_pending_count + $multi_pending_count;
                $badge = $pending_count > 0 ? '<span class="label label-warning pull-right">' . $pending_count . '</span>' : '';
                
                $menu->url(
                    action([\App\Http\Controllers\PendingPaymentController::class, 'index']),
                    __('lang_v1.pending_bank_payments') . ' ' . $badge,
                    [
                        'icon' => 'fa fas fa-university text-yellow', 
                        'active' => request()->segment(1) == 'pending-payments'
                    ]
                )->order(31);
            }
            ...
            if (auth()->user()->can('purchase_n_sell_report.view') || auth()->user()->can('contacts_report.view')
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin) {
                        ...
                        if (auth()->user()->can('ageingreport.view')) {
                            $sub->url (
                        ...
                        if (auth()->user()->can('plastic_bag_report.view')) {
                            $sub->url(
                                action([\App\Http\Controllers\ReportController::class, 'getPlasticBagReport']),
                                __('Plastic Bag Report'),
                                ['icon' => 'fa fas fa-shopping-bag', 'active' => request()->segment(2) == 'plastic-bag-report']
                            );
                        }
                    ...
                    ['icon' => 'fa fas fa-chart-line', 'id' => 'tour_step8']
                )->order(55);
            }
            ...
            if (auth()->user()->can('manage_modules')) {
                $menu->url(action([\App\Http\Controllers\Install\ModulesController::class, 'index']), __('lang_v1.modules'), ['icon' => 'fa fas fa-puzzle-piece', 'active' => request()->segment(1) == 'manage-modules'])->order(60);
            }
            ...
            if (in_array('kitchen', $enabled_modules)) {
                $menu->url(action([\App\Http\Controllers\Restaurant\KitchenController::class, 'index']), __('restaurant.kitchen'), ['icon' => 'fa fas fa-utensils', 'active' => request()->segment(1) == 'modules' && request()->segment(2) == 'kitchen'])->order(70);
            }
            ...
            //Plastic Bag Management menu
            if (auth()->user()->can('plastic_bag.access')) {
                $menu->dropdown(
                    __('Plastic Bag Management'),
                    function ($sub) {
                        if (auth()->user()->can('plastic_bag.access')) {
                            $sub->url(
                                action([\App\Http\Controllers\PlasticBagController::class, 'types']),
                                __('Plastic Bag Types'),
                                ['icon' => 'fa fas fa-tags', 'active' => request()->segment(1) == 'plastic-bag' && request()->segment(2) == 'types']
                            );
                        }
                        if (auth()->user()->can('plastic_bag.create')) {
                            $sub->url(
                                action([\App\Http\Controllers\PlasticBagController::class, 'purchases']),
                                __('Plastic Bag Purchases'),
                                ['icon' => 'fa fas fa-shopping-cart', 'active' => request()->segment(1) == 'plastic-bag' && request()->segment(2) == 'purchases']
                            );
                        }
                        if (auth()->user()->can('plastic_bag.access')) {
                            $sub->url(
                                action([\App\Http\Controllers\PlasticBagController::class, 'stockAdjustments']),
                                __('Stock Adjustments'),
                                ['icon' => 'fa fas fa-exchange-alt', 'active' => request()->segment(1) == 'plastic-bag' && request()->segment(2) == 'stock-adjustments']
                            );
                        }
                        if (auth()->user()->can('plastic_bag.access')) {
                            $sub->url(
                                action([\App\Http\Controllers\PlasticBagController::class, 'stockTransfers']),
                                __('Stock Transfers'),
                                ['icon' => 'fa fas fa-exchange', 'active' => request()->segment(1) == 'plastic-bag' && request()->segment(2) == 'stock-transfers']
                            );
                        }
                        if (auth()->user()->can('plastic_bag.update')) {
                            $sub->url(
                                action([\App\Http\Controllers\PlasticBagController::class, 'adjustPrices']),
                                __('Adjust Prices'),
                                ['icon' => 'fa fas fa-dollar-sign', 'active' => request()->segment(1) == 'plastic-bag' && request()->segment(2) == 'adjust-prices']
                            );
                        }
                    },
            ...
            if (auth()->user()->can('business_settings.access') ||
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules) {
                        ...
                        if (auth()->user()->can('business_settings.access')) {
                            $sub->url(
                                action([\App\Http\Controllers\BusinessController::class, 'getBankTransferSettings']),
                                __('lang_v1.bank_transfer_settings'),
                                ['icon' => 'fa fas fa-university', 'active' => request()->segment(1) == 'bank-transfer-settings']
                            );
                        }
                        if (auth()->user()->can('business_settings.access') && in_array('kitchen', $enabled_modules)) {
                            $sub->url(
                                route('kitchen-settings.index'),
                                __('kitchen.kitchen_settings'),
                                ['icon' => 'fa fas fa-utensils', 'active' => request()->segment(1) == 'kitchen-settings']
                            );
                        }
                    ...
                    ['icon' => 'fa fas fa-cogs', 'id' => 'tour_step3']
                )->order(85);
            }

            /* $menu->url('/subscription', __('Subscriptions'), ['icon' => 'fas fa fa-calendar', 'style' => 'background-color: #FC5733;'])->order(96); */

            // Integrations menu - only visible if user has access_integrations permission
            if (auth()->user()->can('access_integrations')) {
                $menu->url('/integrations', __('Integrations'), ['icon' => 'fa fas fa-plug', 'active' => request()->segment(1) == 'integrations'])->order(95);
            }
