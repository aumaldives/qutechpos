...
class Contact extends Authenticatable
{
    ...
    /**
     * Generate a unique payment token for this contact
     */
    ...
    /**
     * Get the payment link for this contact
     */
    public function getPaymentLinkAttribute()
    {
        ...
        return route('public-payment.show', $this->payment_token);
    }
    ...
    {
        return \App\Transaction::where('contact_id', $this->id)
    ...
    public function getTotalOutstandingAttribute()
    {
        return \App\Transaction::where('contact_id', $this->id)
            ...
            ->where('status', 'final')
            ->whereIn('payment_status', ['due', 'partial'])
            ->sum(\DB::raw('final_total - (SELECT COALESCE(SUM(tp.amount), 0) FROM transaction_payments as tp WHERE tp.transaction_id=transactions.id)'));
    }
    ...
    /**
     * Get the customer group associated with the contact.
     */
