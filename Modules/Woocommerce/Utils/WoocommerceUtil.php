...
class WoocommerceUtil extends Util
{
    ...
    public function get_api_settings($business_id, $location_id = null)
    {
        // If location_id provided, get location-specific settings
        if ($location_id) {
            return WoocommerceLocationSetting::getByLocation($business_id, $location_id);
        }
        
        // Legacy: return business-level settings for backward compatibility
    ...
    /**
     * Creates ModernWooCommerceClient object with v3 API and retry mechanisms
     *
     * @param  int  $business_id
     * @param  int|null  $location_id
     * @return ModernWooCommerceClient
     */
    public function woo_client($business_id, $location_id = null)
    {
        return new ModernWooCommerceClient($business_id, $location_id);
    }

    /**
     * Legacy method for backward compatibility - returns the underlying client
     *
     * @param  int  $business_id
     * @param  int|null  $location_id
     * @return Client
     * @deprecated Use woo_client() which returns ModernWooCommerceClient
     */
    public function legacy_woo_client($business_id, $location_id = null)
    {
        $modernClient = $this->woo_client($business_id, $location_id);
        return $modernClient->getClient();
    }

    public function syncCat($business_id, $data, $type, $new_categories = [])
    {
        try {
            //Modern WooCommerce API client with retry mechanisms and better batch processing
            $modernClient = $this->woo_client($business_id);
            
            $batchResults = $modernClient->withRetry(
                function($client) use ($data, $type) {
                    return $client->post('products/categories/batch', [$type => $data]);
                },
                [
            ...
            //Process results and update woocommerce_cat_id
            if ($type === 'create' && !empty($batchResults->create)) {
                foreach ($batchResults->create as $key => $result) {
                    if (isset($new_categories[$key])) {
                        $category = $new_categories[$key];
                        
                        if ($result->id != 0) {
                            $category->woocommerce_cat_id = $result->id;
                        } elseif (!empty($result->error->data->resource_id)) {
                            $category->woocommerce_cat_id = $result->error->data->resource_id;
                        }
                        
                        $category->save();
                        
                        \Log::info('Category synced to WooCommerce', [
                            'business_id' => $business_id,
                            'category_id' => $category->id,
                            'woocommerce_cat_id' => $category->woocommerce_cat_id
                        ]);
                    }
                }
            }
     ...
     * @return void
     */
    public function syncCategories($business_id, $user_id, $location_id = null)
    {
     ...
     * @return void
     */
    public function syncProducts($business_id, $user_id, $sync_type, $limit = 100, $page = 0, $location_id = null)
    {
        //$limit is zero for console command
        if ($page == 0 || $limit == 0) {
            //Sync Categories
            $this->syncCategories($business_id, $user_id, $location_id);

            //Sync variation attributes
            $this->syncVariationAttributes($business_id, $location_id);
        ...
        $woocommerce_api_settings = $this->get_api_settings($business_id, $location_id);
        $created_data = [];
        $updated_data = [];

        $business_location_id = $location_id ?? $woocommerce_api_settings->location_id;
        $offset = $page * $limit;
     ...
     * @return void
     */
    public function syncVariationAttributes($business_id, $location_id = null)
    {
        $woocommerce = $this->woo_client($business_id, $location_id);
     ...
     * @return void
     */
    public function syncProductVariations($business_id, $sync_type = 'all', $new_woocommerce_product_ids = [], $location_id = null)
    {
        //woocommerce api client object
        $woocommerce = $this->woo_client($business_id, $location_id);
        $woocommerce_api_settings = $this->get_api_settings($business_id, $location_id);
     ...
     * @return void
     */
    public function syncOrders($business_id, $user_id, $location_id = null)
    {
        $last_synced = $this->getLastSync($business_id, 'orders', false);
        $orders = $this->getAllResponse($business_id, 'orders', $location_id, []);
    ...
    /**
     * Splits response to list of 100 and merges all - using modern client with caching and retry
     *
     * @param  int  $business_id
     ...
     * @return array
     */
    public function getAllResponse($business_id, $endpoint, $location_id = null, $params = [])
    {
        try {
            //Modern WooCommerce API client object with v3 API and retry mechanisms
            $modernClient = $this->woo_client($business_id, $location_id);
            
            return $modernClient->getAllItems($endpoint, $params, true);
            
        } catch (\Exception $e) {
            \Log::error('WooCommerce getAllResponse failed', [
                'business_id' => $business_id,
                'endpoint' => $endpoint,
                'error' => $e->getMessage()
            ]);
