...
class WoocommerceWebhookController extends Controller
{
     ...
     * @return bool
     */
    private function validateWebhookSignature(Request $request, $business_id)
    {
        ...
        $woocommerce_settings = json_decode($business->woocommerce_api_settings);
        
        if (empty($woocommerce_settings->woocommerce_wh_oc_secret)) {
            // If no secret is configured, allow the webhook for backward compatibility
            // In production, this should return false for security
            return true;
        }
        
        $webhook_secret = $woocommerce_settings->woocommerce_wh_oc_secret;
    ...
    public function orderCreated(Request $request, $business_id)
    {
        try {
            // Validate webhook signature for security
            if (!$this->validateWebhookSignature($request, $business_id)) {
                return response('Unauthorized webhook signature', 401);
            }
            $payload = $request->getContent();
