...
class SuperadminSettingsController extends Controller
{
    ...
    public function edit()
    {
        ...
        $default_values = [
            ...
            'STRIPE_WEBHOOK_SECRET' => $is_demo ? null : env('STRIPE_WEBHOOK_SECRET'),
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            $system_settings = $request->only(['app_currency_id', 'invoice_business_name', 'email', 'invoice_business_landmark', 'invoice_business_zip', 'invoice_business_state', 'invoice_business_city', 'invoice_business_country', 'package_expiry_alert_days', 'superadmin_register_tc', 'welcome_email_subject', 'welcome_email_body', 'additional_js', 'additional_css', 'offline_payment_details', 'bml_usd_account_name', 'bml_usd_account_number', 'bml_mvr_account_name', 'bml_mvr_account_number', 'mib_usd_account_name', 'mib_usd_account_number', 'mib_mvr_account_name', 'mib_mvr_account_number', 'superadmin_business_tin', 'superadmin_tax_activity_number', 'superadmin_gst_percentage', 'usd_to_mvr_exchange_rate']);
            ...
            foreach ($system_settings as $key => $setting) {
                ...
                // Special handling for exchange rate - update cache when changed
                if ($key === 'usd_to_mvr_exchange_rate' && !empty($setting)) {
                    CurrencyUtil::setManualExchangeRate(floatval($setting));
                }
            }
            ...
            $env_settings = $request->only(['APP_NAME', 'APP_TITLE',
                ...
                'STRIPE_SECRET_KEY', 'STRIPE_WEBHOOK_SECRET', 'PAYPAL_MODE',
