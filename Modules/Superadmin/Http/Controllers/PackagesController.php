...
class PackagesController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            $input = $request->only(['name', 'description', 'location_count', 'user_count', 'product_count', 'invoice_count', 'interval', 'interval_count', 'trial_days', 'price', 'sort_order', 'is_active', 'custom_permissions', 'is_private', 'is_one_time', 'enable_custom_link', 'custom_link',
                'custom_link_text', 'restrict_date', 'restrict_date_after', 'is_per_location_pricing', 'price_per_location', 'min_locations', 'max_locations']);

            $currency = System::getCurrency();

            // Handle per-location pricing logic first
            $input['is_per_location_pricing'] = empty($input['is_per_location_pricing']) ? 0 : 1;

            // Process per-location pricing fields
            if ($input['is_per_location_pricing']) {
                // For per-location packages, price_per_location is the main price
                $input['price_per_location'] = $this->businessUtil->num_uf($input['price_per_location'] ?? 0, $currency);
                $input['min_locations'] = max(1, (int)($input['min_locations'] ?? 1));
                $input['max_locations'] = (int)($input['max_locations'] ?? 0);
                
                // Set base price to 0 for per-location packages (price is calculated dynamically)
                $input['price'] = 0;
                // For per-location packages, location_count becomes 0 (unlimited by default, controlled by customer choice)
                $input['location_count'] = 0;
            } else {
                // Regular package pricing
                $input['price'] = $this->businessUtil->num_uf($input['price'] ?? 0, $currency);
                $input['price_per_location'] = null;
                $input['min_locations'] = 1;
                $input['max_locations'] = 0;
            }
            ...
            $input['restrict_date'] = empty($input['restrict_date']) ? null : $input['restrict_date'];
            $input['restrict_date_after'] = empty($input['restrict_date_after']) ? null : $input['restrict_date_after'];
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            $packages_details = $request->only(['name', 'id', 'description', 'location_count', 'user_count', 'product_count', 'invoice_count', 'interval', 'interval_count', 'trial_days', 'price', 'sort_order', 'is_active', 'custom_permissions', 'is_private', 'is_one_time', 'enable_custom_link', 'custom_link', 'custom_link_text', 'restrict_date', 'restrict_date_after']);
            ...
            $packages_details['restrict_date'] = empty($packages_details['restrict_date']) ? null : $packages_details['restrict_date'];
            $packages_details['restrict_date_after'] = empty($packages_details['restrict_date_after']) ? null : $packages_details['restrict_date_after'];
            ...
            if (! empty($request->input('update_subscriptions'))) {
                $package_details = [
                    ...
                    'name' => $package->name,
                    'restrict_date' => $package->restrict_date,
                    'restrict_date_after' => $package->restrict_date_after,
                ];
