<div class="pos-tab-content">
    ...
    <div class="row">
        ...
        <div class="col-xs-4">
            <div class="form-group">
                {!! Form::label('STRIPE_WEBHOOK_SECRET', __('superadmin::lang.stripe_webhook_secret') . ':') !!}
                {!! Form::text('STRIPE_WEBHOOK_SECRET', $default_values['STRIPE_WEBHOOK_SECRET'], ['class' => 'form-control','placeholder' => __('superadmin::lang.stripe_webhook_secret')]); !!}
            </div>
        </div>
        
        <div class="col-xs-12">
            <div class="alert alert-info">
                <h5><strong>Stripe Webhook Configuration:</strong></h5>
                <p>For Stripe subscriptions to work properly with auto-renewals, you need to configure webhooks in your Stripe dashboard:</p>
                <ol>
                    <li>Go to your <a href="https://dashboard.stripe.com/webhooks" target="_blank">Stripe Dashboard → Webhooks</a></li>
                    <li>Click "Add endpoint"</li>
                    <li>Set the endpoint URL to: <code>{{ url('/webhook/stripe') }}</code></li>
                    ...
                    <li>Copy the webhook signing secret and paste it in the field above</li>
                </ol>
                <p><strong>Note:</strong> Without proper webhook configuration, subscription renewals will not be processed automatically.</p>
            </div>
        </div>
        ...
        <h4>Bank Transfer Settings: <small>(For offline payments)</small></h4>
        <div class="col-xs-12">
            <h5><strong>Bank of Maldives (BML) Accounts:</strong></h5>
        </div>
        <div class="col-xs-6">
            <div class="form-group">
                {!! Form::label('bml_usd_account_name', 'USD Account Name:') !!}
                {!! Form::text('bml_usd_account_name', !empty($settings["bml_usd_account_name"]) ? $settings["bml_usd_account_name"] : 'Ahmed Nazeeh', ['class' => 'form-control']); !!}
            </div>
        </div>
        <div class="col-xs-6">
            <div class="form-group">
                {!! Form::label('bml_usd_account_number', 'USD Account Number:') !!}
                {!! Form::text('bml_usd_account_number', !empty($settings["bml_usd_account_number"]) ? $settings["bml_usd_account_number"] : '7730000153906', ['class' => 'form-control']); !!}
            </div>
        </div>
        <div class="col-xs-6">
            <div class="form-group">
                {!! Form::label('bml_mvr_account_name', 'MVR Account Name:') !!}
                {!! Form::text('bml_mvr_account_name', !empty($settings["bml_mvr_account_name"]) ? $settings["bml_mvr_account_name"] : 'Cleviden Pvt Ltd', ['class' => 'form-control']); !!}
            </div>
        </div>
        <div class="col-xs-6">
            <div class="form-group">
                {!! Form::label('bml_mvr_account_number', 'MVR Account Number:') !!}
                {!! Form::text('bml_mvr_account_number', !empty($settings["bml_mvr_account_number"]) ? $settings["bml_mvr_account_number"] : '7730000757923', ['class' => 'form-control']); !!}
            </div>
        </div>

        <div class="col-xs-12">
            <h5><strong>Maldives Islamic Bank (MIB) Accounts:</strong></h5>
        </div>
        <div class="col-xs-6">
            <div class="form-group">
                {!! Form::label('mib_usd_account_name', 'USD Account Name:') !!}
                {!! Form::text('mib_usd_account_name', !empty($settings["mib_usd_account_name"]) ? $settings["mib_usd_account_name"] : 'Cleviden Pvt Ltd', ['class' => 'form-control']); !!}
            </div>
        </div>
        <div class="col-xs-6">
            <div class="form-group">
                {!! Form::label('mib_usd_account_number', 'USD Account Number:') !!}
                {!! Form::text('mib_usd_account_number', !empty($settings["mib_usd_account_number"]) ? $settings["mib_usd_account_number"] : '90501480029672000', ['class' => 'form-control']); !!}
            </div>
        </div>
        <div class="col-xs-6">
            <div class="form-group">
                {!! Form::label('mib_mvr_account_name', 'MVR Account Name:') !!}
                {!! Form::text('mib_mvr_account_name', !empty($settings["mib_mvr_account_name"]) ? $settings["mib_mvr_account_name"] : 'Cleviden Pvt Ltd', ['class' => 'form-control']); !!}
            </div>
        </div>
        <div class="col-xs-6">
            <div class="form-group">
                {!! Form::label('mib_mvr_account_number', 'MVR Account Number:') !!}
                {!! Form::text('mib_mvr_account_number', !empty($settings["mib_mvr_account_number"]) ? $settings["mib_mvr_account_number"] : '90501480029671000', ['class' => 'form-control']); !!}
            </div>
        </div>
