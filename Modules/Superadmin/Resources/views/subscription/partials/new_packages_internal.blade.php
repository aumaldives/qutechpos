...
$restrictDateAfter = $package->restrict_date_after;
$userCreatedDate = $user_created_date;
@endphp

@if (($restrictDate !== null && strtotime($userCreatedDate) > strtotime($restrictDate)))
@continue
@endif

@if($restrictDateAfter !== null && strtotime($userCreatedDate) < strtotime($restrictDateAfter))
@continue
@endif
	...
	<div class="plan package-{{ $package->interval }}" style="display:none">
		...
		<button type="button" style="padding:3px;">
			...
			<a href="{{ route('business.getRegister') }}?package={{$package->id}}" class="btn btn-block btn-success" style="background-color:#42b87b;border-color:#42b87b;font-weight:bold">
				@if($package->price != 0)
			...
			@else
			<a href="{{action([\Modules\Superadmin\Http\Controllers\SubscriptionController::class, 'pay'], [$package->id])}}" class="btn btn-block btn-success" style="background-color:#42b87b;border-color:#42b87b;font-weight:bold">
				@if($package->price != 0)
		...
		<div class="row">
			<h5 style="margin-top:10px; color:RED; padding:10px; text-align:center;font-weight: bold;">{{$package->description}}</h5>
		</div>
	</div>
	@endif
