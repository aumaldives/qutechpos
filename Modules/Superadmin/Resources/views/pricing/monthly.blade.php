...
<head>
    ...
    <title>Monthly Plans - IsleBooks POS</title>
...
<body>
    ...
    <div class="pricing-container">
        <div class="pricing-header">
            <h1>Monthly Pricing Plans</h1>
            <p>Choose the perfect monthly plan for your business. All plans include our complete POS features with flexible monthly billing.</p>
            
            <div class="plan-toggle">
                <a href="/pricing/monthly" class="active">Monthly</a>
                <a href="/pricing/annual">Annual</a>
            </div>
        ...
        <div class="pricing-grid">
            @php
                // Filter for monthly packages only
                $monthly_packages = collect($packages)
                    ->filter(function($pkg) {
            ...
            @foreach($monthly_packages as $index => $package)
                @php
                    $previous_package = $index > 0 ? $monthly_packages[$index - 1] : null;
                        ...
                        @else
                            <div class="usd-pricing">
                                ...
                                @else
                                    ...
                                    <div class="plan-period">trial period</div>
                                @endif
                            </div>
                            <div class="mvr-pricing" style="display: none;">
                                @if($package->price != 0)
                    ...
                    <div class="card-body">
                        <ul class="features-list">
                            @if($previous_package)
                                ...
                                @if($curr->price >= 40 && (!$prev || $prev->price < 40))
                                    <li>
                                        <i class="fas fa-plus-circle feature-icon plus-icon"></i>
                            ...
                            @else
                                ...
                                <li>
                                    <i class="fas fa-check-circle feature-icon check-icon"></i>
                                    <span><strong>{{ $package->user_count == 0 ? 'Unlimited' : number_format($package->user_count) }} users</strong></span>
                                </li>
                                <li>
                                    <i class="fas fa-check-circle feature-icon check-icon"></i>
    ...
    <script>
        ...
        function updatePrice(packageId) {
            ...
            const card = input.closest('.card-header');
            card.querySelector('.total-usd').textContent = `$${totalUSD.toFixed(2)}`;
            card.querySelector('.total-mvr').textContent = `MVR ${Math.round(totalMVR)}`;
        }
