...
class DataController extends Controller
{
    ...
    public function modifyAdminMenu()
    {
        ...
        if ($is_essentials_enabled) {
            Menu::modify('admin-sidebar-menu', function ($menu) use ($business_id) {
                // Check if user has any HR-related permissions before showing HRM menu
                $user = auth()->user();
                $has_hr_permissions = $user->can('essentials.crud_leave_type') ||
                    $user->can('essentials.crud_all_leave') || 
                    $user->can('essentials.crud_own_leave') ||
                    $user->can('essentials.approve_leave') ||
                    ...
                    $user->can('essentials.allow_users_for_attendance_from_api') ||
                    $user->can('essentials.view_allowance_and_deduction') ||
                    $user->can('essentials.add_allowance_and_deduction') ||
                    $user->can('essentials.crud_department') ||
                    $user->can('essentials.crud_designation') ||
                    $user->can('essentials.view_all_payroll') ||
                    $user->can('essentials.create_payroll') ||
                    $user->can('essentials.update_payroll') ||
                    $user->can('essentials.delete_payroll') ||
                    $user->hasRole('Admin#' . $business_id);


                if ($has_hr_permissions) {
                    $menu->url(
    ...
    /**
     * Returns dashboard widgets for the Essentials module
     *
     * @return array
     */
    public function dashboard_widget()
    {
        ...
        // Safely get business_id from session
        $business_id = null;
        try {
            $business_id = request()->session()->get('user.business_id');
        } catch (Exception $e) {
            // If session is not available, try to get from auth user
            $business_id = auth()->user()->business_id ?? null;
        }
        
        $user = auth()->user();
        $is_admin = $user->can('essentials.approve_leave') || 
                   $user->can('essentials.crud_all_leave') ||
                   $user->can('essentials.crud_all_overtime') ||
                   $user->hasRole('Admin#' . $business_id);
        ...
        if ($is_admin && $business_id) {
            ...
            $pending_overtime_count = EssentialsOvertimeRequest::where('business_id', $business_id)
                ->where('status', 'pending')
                ->count();

            // Add Leave Requests Awaiting Approval widget (always show)
            $leave_url = action([\Modules\Essentials\Http\Controllers\EssentialsLeaveController::class, 'index'], ['status' => 'pending']);
            $widgets[] = [
                'position' => 'after_sale_purchase_totals',
                'widget' => '
                    <div class="dashboard-card-5-col">
                        <a href="' . $leave_url . '" style="color: inherit; text-decoration: none;">
                            <div class="modern-dashboard-card">
                                <div class="modern-card-header">
                                    <div class="modern-card-icon icon-leave">
                                        <i class="fas fa-user-times"></i>
                                    </div>
                                    <div class="modern-card-content">
                                        <h3 class="modern-card-title">' . __('essentials::lang.leave_requests_awaiting_approval') . '</h3>
                                        <p class="modern-card-value">' . $pending_leaves_count . '</p>
                                    </div>
                                </div>
                            </div>
                        </a>
            ...
            // Add OT Requests Awaiting Approval widget (always show)
            $overtime_url = action([\Modules\Essentials\Http\Controllers\OvertimeController::class, 'index'], ['status' => 'pending']);
            $widgets[] = [
                'position' => 'after_sale_purchase_totals',
                'widget' => '
                    <div class="dashboard-card-5-col">
                        <a href="' . $overtime_url . '" style="color: inherit; text-decoration: none;">
                            <div class="modern-dashboard-card">
                                <div class="modern-card-header">
                                    <div class="modern-card-icon icon-ot">
                                        <i class="fas fa-clock"></i>
                                    </div>
                                    <div class="modern-card-content">
                                        <h3 class="modern-card-title">' . __('essentials::lang.ot_requests_awaiting_approval') . '</h3>
                                        <p class="modern-card-value">' . $pending_overtime_count . '</p>
                                    </div>
                                </div>
                            </div>
                        </a>
