...
class EssentialsLeaveController extends Controller
{
    ...
    private function __addLeave($input, $user_id = null)
    {
        ...
        // Validate leave balance before creating leave
        $business_id = $input['business_id'];
        $user_id_final = $input['user_id'];
        $leave_type_id = $input['essentials_leave_type_id'];
        ...
        $user_leave_balances = $this->essentialsUtil->getUserLeaveBalances($user_id_final, $business_id);
        
        // Find the specific leave type balance
        $leave_type_balance = null;
        foreach ($user_leave_balances as $balance) {
            $leave_type = EssentialsLeaveType::find($leave_type_id);
            if ($balance['leave_type'] === $leave_type->leave_type) {
                $leave_type_balance = $balance;
                break;
            }
        }
        
        // Check if leave type has balance validation enabled and if balance is insufficient
        if ($leave_type_balance && $requested_days > $leave_type_balance['remaining_count']) {
            // Check settings to see if exceeding limits is allowed
            $settings = $this->essentialsUtil->getEssentialsSettings($business_id);
            $allow_exceeding_limits = !empty($settings['allow_exceeding_leave_limits']);
            
            if (!$allow_exceeding_limits) {
                $user = User::find($user_id_final);
                $user_name = $user ? $user->user_full_name : 'Employee';
                throw new \Exception("Leave request denied. {$user_name} has only {$leave_type_balance['remaining_count']} {$leave_type_balance['leave_type']} days remaining, but requested {$requested_days} days.");
            }
        }
    ...
    /**
     * Check leave balance for validation
     *
     * @param  Request  $request
     * @return Response
     */
    public function checkLeaveBalance(Request $request)
    {
        ...
        try {
            ...
            // Get user's leave balances
            $user_leave_balances = $this->essentialsUtil->getUserLeaveBalances($user_id, $business_id);
            ...
            if (!$leave_type_balance) {
                return response()->json([
                    'success' => true,
                    'can_apply' => true,
                    'message' => 'No balance limit configured for this leave type',
                    'balance_info' => null
                ]);
            }
            ...
            $can_apply = $requested_days <= $leave_type_balance['remaining_count'] || $allow_exceeding_limits;
            $message = '';

            if ($requested_days > $leave_type_balance['remaining_count']) {
                if ($allow_exceeding_limits) {
                    $excess_days = $requested_days - $leave_type_balance['remaining_count'];
                    $message = "Warning: You are requesting {$excess_days} days more than your remaining balance of {$leave_type_balance['remaining_count']} days.";
                } else {
                    $message = "Insufficient leave balance. You have {$leave_type_balance['remaining_count']} days remaining but requested {$requested_days} days.";
                }
            } else {
                $remaining_after = $leave_type_balance['remaining_count'] - $requested_days;
                $message = "Request valid. You will have {$remaining_after} {$leave_type->leave_type} days remaining after this request.";
            }
            ...
            return response()->json([
                ...
                'message' => $message,
                'balance_info' => [
                    'leave_type' => $leave_type_balance['leave_type'],
                    'max_count' => $leave_type_balance['max_count'],
                    'used_count' => $leave_type_balance['used_count'],
                    'remaining_count' => $leave_type_balance['remaining_count'],
                    'requested_days' => $requested_days,
                    'allow_exceeding' => $allow_exceeding_limits
                ]
            ]);
