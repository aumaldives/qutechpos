...
class AttendanceController extends Controller
{
    ...
    /**
     * Handle OT In/Out toggle
     */
    public function otInOut(Request $request)
    {
        $business_id = $request->session()->get('user.business_id');
        $user_id = auth()->user()->id;
        ...
        try {
            ...
            if ($type == 'ot_in') {
                // Check if user already has an active OT session
                $existing_session = OtSession::getActiveSession($user_id, $business_id);
                
                if ($existing_session) {
                    $output = [
                        'success' => false,
                        'msg' => __('essentials::lang.already_ot_in')
                    ];
                    return $output;
                }

                // Create new OT session
                $ot_session = OtSession::create([
                    'user_id' => $user_id,
                    'business_id' => $business_id,
                    'start_time' => now(),
                    'start_note' => $request->input('ot_in_note'),
                    'status' => 'active'
                ]);

                $output = [
                    'success' => true,
                    'msg' => __('essentials::lang.ot_session_started'),
                    'type' => 'ot_in',
                    'session_id' => $ot_session->id
                ];

            } elseif ($type == 'ot_out') {
                // Find active OT session
                $ot_session = OtSession::getActiveSession($user_id, $business_id);
                
                if (!$ot_session) {
                    $output = [
                        'success' => false,
                        'msg' => __('essentials::lang.no_active_ot_session')
                    ];
                    return $output;
                }
                ...
                $ot_session->update([
                    'end_time' => now(),
                    'end_note' => $request->input('ot_out_note'),
                    'status' => 'completed'
                ]);

                // Auto-submit as draft overtime request if outside normal work hours
                $is_outside_hours = $ot_session->isOutsideWorkHours();
                $duration_minutes = $ot_session->getDurationInMinutes();
                
                // Get minimum overtime duration from settings
                $essentials_settings = $this->essentialsUtil->getEssentialsSettings();
                $minimum_minutes = $essentials_settings['overtime_minimum_minutes'] ?? 15;
                
                if ($is_outside_hours && $duration_minutes >= $minimum_minutes) {
                    $this->createDraftOvertimeRequest($ot_session);
                    $msg = __('essentials::lang.ot_session_ended_draft_created');
                } else {
                    $msg = __('essentials::lang.ot_session_ended');
                    
                    // Add debug info for why draft wasn't created
                    if (!$is_outside_hours) {
                        $msg .= ' (During work hours)';
                    } elseif ($duration_minutes < $minimum_minutes) {
                        $msg .= ' (Duration too short: ' . round($duration_minutes, 1) . ' min, minimum: ' . $minimum_minutes . ' min)';
                    }
                }
                ...
                $output = [
                    ...
                    'msg' => $msg,
                    'type' => 'ot_out',
                    'duration' => round($ot_session->getDurationInHours(), 2)
                ];
            }
    ...
    /**
     * Create draft overtime request from OT session
     */
    private function createDraftOvertimeRequest($ot_session)
    {
        // Import the OvertimeController to use its logic
        $overtimeController = app(\Modules\Essentials\Http\Controllers\OvertimeController::class);
        
        $request_data = [
            'date' => $ot_session->start_time->format('Y-m-d'),
            'start_time' => $ot_session->start_time->format('H:i'),
            'end_time' => $ot_session->end_time->format('H:i'),
            'reason' => 'Auto-created from OT session. ' . ($ot_session->start_note ? 'Start: ' . $ot_session->start_note . '. ' : '') . ($ot_session->end_note ? 'End: ' . $ot_session->end_note : ''),
            'status' => 'draft'
        ];

        // Create overtime request directly using the OvertimeRequest model
        \Modules\Essentials\Entities\EssentialsOvertimeRequest::create([
            'business_id' => $ot_session->business_id,
            'user_id' => $ot_session->user_id,
            'overtime_date' => $request_data['date'],
            'start_time' => $request_data['start_time'],
            'end_time' => $request_data['end_time'],
            'reason' => $request_data['reason'],
            'status' => 'draft',
            'hours_requested' => $ot_session->getDurationInHours()
        ]);
    }
}
