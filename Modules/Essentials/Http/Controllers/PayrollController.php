    ...
    public function create()
    {
        ...
        if (! empty($add_payroll_for)) {
            ...
            foreach ($employees as $employee) {
                ...
                // Calculate shift-aware working days and hourly rate
                $settings = $this->essentialsUtil->getEssentialsSettings($business_id);
                if (!empty($settings['auto_calculate_hourly_in_payroll'])) {
                    $hourly_rate = $this->essentialsUtil->calculateHourlyRate($employee->id, $employee->essentials_salary, $month, $year, $business_id);
                    $scheduled_working_days = $this->essentialsUtil->calculateWorkingDaysForUser($employee->id, $month, $year);
                    
                    // Use precise shift hours if employee has a shift assigned
                    if (!empty($employee->essentials_shift_id)) {
                        $scheduled_working_hours = $this->essentialsUtil->calculateShiftHours($employee->essentials_shift_id, $month, $year);
                    } else {
                        $scheduled_working_hours = $scheduled_working_days * 8; // Default 8 hours per day
                    }
                    
                    $payrolls[$employee->id]['hourly_rate'] = $hourly_rate;
                    $payrolls[$employee->id]['scheduled_working_days'] = $scheduled_working_days;
                    $payrolls[$employee->id]['scheduled_working_hours'] = $scheduled_working_hours;
                    $payrolls[$employee->id]['attendance_vs_scheduled'] = $payrolls[$employee->id]['total_work_duration'] . ' / ' . $scheduled_working_hours;
                    
                    // Calculate attendance efficiency
                    $attendance_efficiency = $scheduled_working_hours > 0 ? 
                        round(($payrolls[$employee->id]['total_work_duration'] / $scheduled_working_hours) * 100, 1) : 0;
                    $payrolls[$employee->id]['attendance_efficiency'] = $attendance_efficiency;
                    
                    // Add shift-aware validation
                    $validation = $this->essentialsUtil->getShiftAwarePayrollValidation(
                        $employee->id, $month, $year, $payrolls[$employee->id]['total_work_duration'], $business_id
                    );
                    $payrolls[$employee->id]['has_attendance_warning'] = !$validation['is_valid'];
                    $payrolls[$employee->id]['attendance_status'] = $validation['status'];
                    $payrolls[$employee->id]['variance_hours'] = $validation['variance'];
                    $payrolls[$employee->id]['variance_percentage'] = $validation['variance_percentage'];
                }
                ...
                $overtime_requests = \Modules\Essentials\Entities\EssentialsOvertimeRequest::where('business_id', $business_id)
                    ->where('user_id', $employee->id)
                    ->where('status', 'approved')
                    ->where('is_processed_in_payroll', 0)
                    ->whereBetween('overtime_date', [$start_date, $end_date->format('Y-m-d')])
                    ->get();
                ...
                foreach ($overtime_requests as $overtime) {
                    $total_overtime_amount += $overtime->total_amount;
                    $overtime_hours += $overtime->approved_hours;
                    $overtime_breakdown[] = [
                        'date' => $overtime->overtime_date,
                        'hours' => $overtime->approved_hours,
                        'type' => $overtime->overtime_type,
                        'amount' => $overtime->total_amount,
                        'multiplier' => $overtime->multiplier_rate
                    ];
                }

                if ($total_overtime_amount > 0) {
                    $payrolls[$employee->id]['allowances']['allowance_names'][] = __('essentials::lang.overtime_allowance');
                    $payrolls[$employee->id]['allowances']['allowance_amounts'][] = $total_overtime_amount;
                    $payrolls[$employee->id]['allowances']['allowance_types'][] = 'fixed';
                    $payrolls[$employee->id]['allowances']['allowance_percents'][] = 0;
                    
                    // Store overtime details for reference
                    $payrolls[$employee->id]['overtime_details'] = [
                        'total_hours' => $overtime_hours,
                        'total_amount' => $total_overtime_amount,
                        'breakdown' => $overtime_breakdown,
                        'request_ids' => $overtime_requests->pluck('id')->toArray()
                    ];
                    
                    // Store request IDs separately for processing
                    $payrolls[$employee->id]['overtime_request_ids'] = $overtime_requests->pluck('id')->toArray();
                }

                // Calculate absent day deductions
                $absent_day_deductions = $this->essentialsUtil->calculateAbsentDayDeductions($employee->id, $month, $year, $business_id);
                if (!empty($absent_day_deductions) && is_array($absent_day_deductions) && $absent_day_deductions['total_deduction'] > 0) {
                    $payrolls[$employee->id]['deductions']['deduction_names'][] = __('essentials::lang.absent_day_deduction');
                    $payrolls[$employee->id]['deductions']['deduction_amounts'][] = $absent_day_deductions['total_deduction'];
                    $payrolls[$employee->id]['deductions']['deduction_types'][] = 'fixed';
                    $payrolls[$employee->id]['deductions']['deduction_percents'][] = 0;
                    
                    // Store absent day details for reference
                    $payrolls[$employee->id]['absent_day_details'] = $absent_day_deductions;
                }

                // Calculate late penalty deductions
                $late_penalty_deductions = $this->essentialsUtil->calculateLatePenaltyDeductions($employee->id, $month, $year, $business_id);
                if (!empty($late_penalty_deductions) && is_array($late_penalty_deductions) && $late_penalty_deductions['total_deduction'] > 0) {
                    $payrolls[$employee->id]['deductions']['deduction_names'][] = __('essentials::lang.late_penalty_deduction');
                    $payrolls[$employee->id]['deductions']['deduction_amounts'][] = $late_penalty_deductions['total_deduction'];
                    $payrolls[$employee->id]['deductions']['deduction_types'][] = 'fixed';
                    $payrolls[$employee->id]['deductions']['deduction_percents'][] = 0;
                    
                    // Store late penalty details for reference
                    $payrolls[$employee->id]['late_penalty_details'] = $late_penalty_deductions;
                }
            }

            $action = 'create';

            // Generate payroll summary for admin insights
            $payroll_summary = $this->essentialsUtil->generatePayrollSummaryReport($payrolls, $business_id);
            
            // Generate recommendations for each employee (limit to prevent performance issues)
            $recommendations = [];
            $recommendation_count = 0;
            foreach ($payrolls as $employee_id => $payroll_data) {
                if ($recommendation_count >= 5) break; // Limit to 5 employees with recommendations
                
                $employee_recommendations = $this->essentialsUtil->getPayrollRecommendations($employee_id, $month, $year, $business_id);
                if (!empty($employee_recommendations)) {
            ...
            return view('essentials::payroll.create')
                    ->with(compact('month_name', 'transaction_date', 'year', 'payrolls', 'action', 'location', 'payroll_summary', 'recommendations'));
        } else {
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach ($payrolls as $key => $payroll) {
                ...
                // Store overtime request IDs before removing other data
                $overtime_request_ids = $payroll['overtime_request_ids'] ?? [];
                
                unset($payroll['allowance_names'], $payroll['allowance_types'], $payroll['allowance_percent'], $payroll['allowance_amounts'], $payroll['deduction_names'], $payroll['deduction_types'], $payroll['deduction_percent'], $payroll['deduction_amounts'], $payroll['total'], $payroll['overtime_request_ids'], $payroll['overtime_details'], $payroll['absent_day_details'], $payroll['late_penalty_details']);

                $transaction = Transaction::create($payroll);
                $transaction_ids[] = $transaction->id;

                // Mark overtime requests as processed
                if (!empty($overtime_request_ids)) {
                    \Modules\Essentials\Entities\EssentialsOvertimeRequest::whereIn('id', $overtime_request_ids)
                        ->update([
