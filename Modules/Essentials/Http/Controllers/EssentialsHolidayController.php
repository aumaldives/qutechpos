    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            try {
                ...
                // Fallback: assume dates are already in Y-m-d format or convert directly
                $input['start_date'] = \Carbon::parse($input['start_date'])->format('Y-m-d');
                $input['end_date'] = \Carbon::parse($input['end_date'])->format('Y-m-d');
                
                \Log::warning('Holiday date conversion fallback used', [
                    'original_start' => $request->input('start_date'),
                    'original_end' => $request->input('end_date'),
                    'converted_start' => $input['start_date'],
                    'converted_end' => $input['end_date'],
                    'error' => $date_error->getMessage()
                ]);
            }
            ...
            $output = ['success' => false,
                'msg' => 'Validation error: ' . implode(', ', $v->validator->errors()->all()),
            ];
        } catch (\Exception $e) {
            \Log::emergency('Holiday Creation Error - File:'.$e->getFile().' Line:'.$e->getLine().' Message:'.$e->getMessage().' Input:'.json_encode($request->all()));
