...
$(document).ready( function () {
    ...
    $(document).on('change', '.essentials_duration, .essentials_amount_per_unit_duration, input.allowance, input.deduction, input.percent', function() {
        ...
        // Validate shift-aware calculations when duration changes
        if ($(this).hasClass('essentials_duration')) {
            validateShiftAwareDuration(id);
        }
    ...
    // Add shift-aware validation function
    function validateShiftAwareDuration(id) {
        let actualHours = __read_number($("input#essentials_duration_" + id));
        let row = $("tr[data-id='" + id + "']");
        
        // Get scheduled hours from data attributes if available
        let scheduledHours = row.data('scheduled-hours') || 0;
        let attendanceWarning = row.data('attendance-warning') || false;
        
        if (scheduledHours > 0) {
            let variance = actualHours - scheduledHours;
            let variancePercent = Math.abs(variance) / scheduledHours * 100;
            
            // Remove existing warnings
            row.find('.duration-validation-warning').remove();
            
            // Add warning if variance is significant (>15%)
            if (variancePercent > 15) {
                let warningMessage = '';
                let warningClass = 'alert-warning';
                
                if (variance > 0) {
                    warningMessage = 'Modified hours exceed scheduled hours by ' + variance.toFixed(1) + ' hrs (' + variancePercent.toFixed(1) + '% overtime)';
                    warningClass = 'alert-info';
                } else {
                    warningMessage = 'Modified hours are ' + Math.abs(variance).toFixed(1) + ' hrs below scheduled hours (' + variancePercent.toFixed(1) + '% undertime)';
                    warningClass = 'alert-warning';
                }
                
                let warningHtml = '<div class="alert ' + warningClass + ' alert-sm duration-validation-warning" style="margin-top: 5px; padding: 5px;">' +
                                '<small><i class="fa fa-info-circle"></i> ' + warningMessage + '</small></div>';
                
                $("input#essentials_duration_" + id).closest('td').append(warningHtml);
            }
        }
    }
    ...
    $("table#payroll_table tbody tr").each(function() {
        let id = $(this).data('id');
        // Store scheduled hours and warning status as data attributes for JS access
        @foreach($payrolls as $employee_id => $payroll_data)
            @if(!empty($payroll_data['scheduled_working_hours']))
                $("tr[data-id='{{$employee_id}}']").data('scheduled-hours', {{$payroll_data['scheduled_working_hours']}});
                $("tr[data-id='{{$employee_id}}']").data('attendance-warning', {{!empty($payroll_data['has_attendance_warning']) ? 'true' : 'false'}});
            @endif
        @endforeach
        
        validateShiftAwareDuration($(this).data('id'));
    });
});
</script>