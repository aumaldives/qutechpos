            ...
            <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                <ul class="nav navbar-nav">
                    ...
                    @if(auth()->user()->can('essentials.crud_all_overtime') || auth()->user()->can('superadmin'))
                        <!-- Admin users see both options -->
                        <li @if(request()->segment(2) == 'overtime' && request()->get('view', 'my') == 'my') class="active" @endif><a href="{{action([\Modules\Essentials\Http\Controllers\OvertimeController::class, 'index'], ['view' => 'my'])}}">@lang('essentials::lang.my_overtime_requests')</a></li>
                        <li @if(request()->segment(2) == 'overtime' && request()->get('view') == 'all') class="active" @endif><a href="{{action([\Modules\Essentials\Http\Controllers\OvertimeController::class, 'index'], ['view' => 'all'])}}">@lang('essentials::lang.overtime_approvals')</a></li>
                    @elseif(auth()->user()->can('essentials.crud_own_overtime'))
                        <!-- Regular employees see only their own -->
                        <li @if(request()->segment(2) == 'overtime') class="active" @endif><a href="{{action([\Modules\Essentials\Http\Controllers\OvertimeController::class, 'index'], ['view' => 'my'])}}">@lang('essentials::lang.my_overtime_requests')</a></li>
                    @else
                        <!-- Fallback for any employee with essentials access -->
                        <li @if(request()->segment(2) == 'overtime') class="active" @endif><a href="{{action([\Modules\Essentials\Http\Controllers\OvertimeController::class, 'index'], ['view' => 'my'])}}">@lang('essentials::lang.my_overtime_requests')</a></li>
                    @endif
