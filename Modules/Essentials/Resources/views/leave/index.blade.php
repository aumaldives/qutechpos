    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#add_leave_modal').on('shown.bs.modal', function(e) {
                ...
                $('#leave_balance_info').remove();
            });

            // Add leave balance checking
            function checkLeaveBalance() {
                var leave_type_id = $('#essentials_leave_type_id').val();
                var start_date = $('#start_date').val();
                var end_date = $('#end_date').val();
                var user_id = $('#employees').val(); // For admin selecting employees
                ...
                // If admin is selecting multiple employees, check for the first one
                if (user_id && Array.isArray(user_id) && user_id.length > 0) {
                    user_id = user_id[0];
                }

                $.ajax({
                    url: '/hrm/check-leave-balance',
                    method: 'POST',
                    data: {
                    ...
                    success: function(response) {
                        ...
                        if (response.success && response.balance_info) {
                            var alertClass = response.can_apply ? 'alert-info' : 'alert-danger';
                            var balanceHtml = '<div id="leave_balance_info" class="alert ' + alertClass + ' mt-2">' +
                                '<strong>Balance Check:</strong> ' + response.message;
                            
                            if (response.balance_info) {
                                balanceHtml += '<br><small>' +
                                    'Max: ' + response.balance_info.max_count + ' days | ' +
                                    'Used: ' + response.balance_info.used_count + ' days | ' +
                                    'Remaining: ' + response.balance_info.remaining_count + ' days | ' +
                                    'Requested: ' + response.balance_info.requested_days + ' days' +
                                    '</small>';
                            }
                            
                            balanceHtml += '</div>';
                            
                            $('#add_leave_form .modal-body').append(balanceHtml);
                            
                            // Disable/enable submit button based on validation
                            $('.add-leave-btn').prop('disabled', !response.can_apply);
                        }
                    },
                    error: function(xhr) {
                        console.log('Leave balance check failed:', xhr.responseJSON);
                    }
                });
            }

            // Trigger balance check when form fields change
            $(document).on('change', '#essentials_leave_type_id, #start_date, #end_date, #employees', function() {
                setTimeout(checkLeaveBalance, 300); // Small delay to ensure all fields are updated
            });
